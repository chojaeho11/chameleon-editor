...
(function AiNbTools(){
    ...
    function _meEnsureArtboardMask() {
        ...
        stage.appendChild(m);
        // 2026-07-19: 이전에 추가했던 별도 경계선 레이어(.me-artboard-line) 원복 — 남아있으면 제거.
        try {
            var _old = stage.querySelector(':scope > .me-artboard-line');
    ...
    function _meAiEnsureModal() {
        ...
        // 2026-07-20: 모달 안의 수정 패널은 제거됨 — 수정은 대지 아래 [수정하기] 에서 한다.
    ...
    async function _meHeroGoProduct(code, imgUrl) {
        try {
            ...
            // 2026-07-20 [중요] _isAiBg 표식을 반드시 달아야 한다.
            //   안 달면 _meGalRemovePreview 가 못 찾아 다음 수정 때 이전 배경이 안 지워지고,
            //   화면(DOM 순서)엔 새 그림 / 저장·내보내기(items 순서)엔 옛 그림이 나오는
            //   위 6786 주석의 그 버그가 그대로 재현된다 (실제로 '자세히보기' 에서 옛 그림이 나왔다).
    ...
    function _meShowLargeImage(src) {
        if (!src) return;
        ...
        ov.innerHTML = '<img src="' + src + '" style="max-width:96%; max-height:92%; object-fit:contain; border-radius:8px;">'
    ...
    window._meFixRun = async function () {
        ...
        try {
            ...
            if (t) t.value = '';
            // 이력을 볼 수 있게 패널은 닫지 않는다 (안내 문구만 잠시 후 정리)
            setTimeout(function () { if (msg) msg.style.display = 'none'; }, 2600);
        } catch (e) {
    ...
    window._meAiOptOutGallery = async function (btn) {
        if (btn) { btn.disabled = true; btn.style.opacity = '0.6'; }
        // 갤러리 등록은 비동기(fire&forget)라 방금 눌렀을 때 아직 id 가 안 잡혔을 수 있다 → 잠깐 기다린다.
        if (!_meLastGalleryId) {
            for (var _w = 0; _w < 30 && !_meLastGalleryId; _w++) { await new Promise(function (r) { setTimeout(r, 200); }); }
        }
        if (!_meLastGalleryId) { // 그래도 없으면 (등록 실패/PII pending 등) — 공개 안 된 것으로 안내
            if (btn) { btn.textContent = _meAiTr('공개되지 않아요', '公開されません', 'Kept private'); btn.style.background = '#f0fdf4'; btn.style.borderColor = '#86efac'; btn.style.color = '#15803d'; btn.style.opacity = '1'; }
            return;
        }
