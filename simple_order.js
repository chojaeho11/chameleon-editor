...
@media (max-width: 768px) {
    ...
    .so-body > .so-right span,
    .so-body > .so-right div,
    .so-body > .so-right h1,
    .so-body > .so-right h2,
    .so-body > .so-right h3,
  ...
  <div class="so-modal">
    ...
    <div class="so-body">
      ...
      <div class="so-right">
        ...
        <!-- 2026-06-05: 허니콤 포토존 (나무조형물·동화책·큐브·룰렛) 6종 — 카드 그리드 -->
        <div class="so-section" id="soPhotozoneVariantsSec" style="display:none;">
          <div class="so-section-title">${tr('포토존 종류 선택', 'フォトゾーン種類選択', 'Choose photo zone type')} <span style="font-size:11px; color:#64748b; font-weight:600;">${tr('카드를 눌러 종류 변경', 'カードで切替', 'Click to switch')}</span></div>
        ...
        <!-- 2026-06-06: 현수막 9종 (초저가/UV/친환경/어깨띠/라텍스/깃발/텐트천/방염/족자) — 카드 그리드 -->
        <div class="so-section" id="soPlacardVariantsSec" style="display:none;">
          <div class="so-section-title">${tr('현수막 종류 선택', '横断幕 種類選択', 'Choose placard type')} <span style="font-size:11px; color:#64748b; font-weight:600;">${tr('카드를 눌러 종류 변경', 'カードで切替', 'Click to switch')}</span></div>
        ...
        <!-- 2026-06-06: 시트지 4종 (탈부착/안개/차량랩핑/글씨커팅) — 카드 그리드 -->
        <div class="so-section" id="soVinylVariantsSec" style="display:none;">
          <div class="so-section-title">${tr('시트지 종류 선택', 'シート種類選択', 'Choose vinyl type')} <span style="font-size:11px; color:#64748b; font-weight:600;">${tr('카드를 눌러 종류 변경', 'カードで切替', 'Click to switch')}</span></div>
        ...
        <!-- 2026-06-13: 칼선작업 섹션은 가격 박스 바로 위로 이동 (아래 soPriceBox 직전 정의) -->
        ...
        <div class="so-section" id="soRealPrintSection" style="display:none;">
          ...
          <div style="display:grid; grid-template-columns:repeat(2,1fr); gap:8px; margin-bottom:14px;">
            <button type="button" class="so-rp-width-btn active" data-width="90" onclick="window._soPickRealPrintWidth(this)"
              style="padding:14px 10px; border:2px solid #0f172a; background:#0f172a; color:#fff; border-radius:12px; cursor:pointer; font-family:inherit; transition:all 0.15s ease;">
              <div style="font-size:14px; font-weight:900;">90폭 <span style="font-weight:600; opacity:0.85;">(90cm)</span></div>
              <div style="font-size:11px; opacity:0.8; margin-top:3px;">${tr('표준 단가', '標準価格', 'Standard rate')}</div>
            </button>
            <button type="button" class="so-rp-width-btn" data-width="127" onclick="window._soPickRealPrintWidth(this)"
              style="padding:14px 10px; border:2px solid #e2e8f0; background:#fff; color:#334155; border-radius:12px; cursor:pointer; font-family:inherit; transition:all 0.15s ease;">
              <div style="font-size:14px; font-weight:900;">127폭 <span style="font-weight:600; opacity:0.85;">(127cm)</span></div>
          ...
          <div id="soRpCoatingWrap" style="display:none; margin-bottom:14px;">
            ...
            <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:6px;">
              <button type="button" class="so-rp-coat-btn active" data-coat="uv" onclick="window._soPickRealPrintCoating(this)"
                style="padding:12px 8px; border:2px solid #0f172a; background:#0f172a; color:#fff; border-radius:10px; cursor:pointer; font-family:inherit;">
                <div style="font-size:13px; font-weight:900;">UV ${tr('인쇄', '印刷', 'print')}</div>
                <div style="font-size:10px; opacity:0.85; margin-top:2px; color:#86efac;">${tr('무료', '無料', 'Free')}</div>
              </button>
          ...
          <!-- 안내 -->
          <div style="padding:14px 16px; background:linear-gradient(135deg,#fef3c7,#fde68a); border:1.5px solid #f59e0b; border-radius:12px; font-size:13px; font-weight:700; color:#78350f; line-height:1.8;">
            <div style="font-size:14px; font-weight:900; margin-bottom:8px; color:#92400e; display:flex; align-items:center; gap:6px;">
        ...
        <div class="so-section" id="soTshirtPrintAreaSection" style="display:none;">
          ...
          <div id="soTshirtPrintAreaGrid" style="display:grid; grid-template-columns:repeat(3, 1fr); gap:8px;">
            <button type="button" class="so-tpa-btn active" data-area="front_logo" data-area-fee="3000" onclick="window._soToggleTshirtPrintArea(this)" style="border:2px solid #0f172a; background:#0f172a; border-radius:12px; padding:8px 6px; cursor:pointer; display:flex; flex-direction:column; align-items:center; gap:6px; transition:border-color 0.15s ease, background 0.15s ease; font-family:inherit;">
              ...
              <span style="font-size:10.5px; font-weight:800; color:#fbbf24;">+${fmtPrice(3000)}<span style="font-size:9.5px; opacity:0.7; font-weight:700;"> · ${tr('장당', '枚あたり', '/pc')}</span></span>
            </button>
            <button type="button" class="so-tpa-btn" data-area="front_full" data-area-fee="8000" onclick="window._soToggleTshirtPrintArea(this)" style="border:2px solid #e2e8f0; background:#fff; border-radius:12px; padding:8px 6px; cursor:pointer; display:flex; flex-direction:column; align-items:center; gap:6px; transition:border-color 0.15s ease, background 0.15s ease; font-family:inherit;">
              ...
              <span style="font-size:10.5px; font-weight:800; color:#dc2626;">+${fmtPrice(8000)}<span style="font-size:9.5px; opacity:0.7; font-weight:700;"> · ${tr('장당', '枚あたり', '/pc')}</span></span>
            </button>
            <button type="button" class="so-tpa-btn" data-area="back_full" data-area-fee="8000" onclick="window._soToggleTshirtPrintArea(this)" style="border:2px solid #e2e8f0; background:#fff; border-radius:12px; padding:8px 6px; cursor:pointer; display:flex; flex-direction:column; align-items:center; gap:6px; transition:border-color 0.15s ease, background 0.15s ease; font-family:inherit;">
        ...
        <!-- 2026-06-13: 칼선작업 (배경제거 + 누끼 + 받침) — 가격 박스 바로 위. 한 종류씩 카트 담기이므로 개수 입력 제거 (수량 자동) -->
...
<div id="soCheckoutOverlay" class="so-co-overlay" style="display:none;">
  <div class="so-co-card">
    ...
    function recalc() {
        ...
        let qty, subtotal;
        // 2026-06-01: "현재 폼" 이 비어있는 초안 (큐에 라인이 있는데 새 라인 시작 전) 인지 판정.
        ...
        let wallShapeFee = (state.isWall && state.wallShapeFee) ? state.wallShapeFee : 0;
        // 2026-06-01: 큐에 라인이 있는데 현재 폼이 빈 초안 — 현재 라인 합계 모두 0
        if (_curIsDraft) {
            subtotal = 0;
            heightExtra = 0;
            wallShapeFee = 0;
        ...
        if (tshirtPrintFee > 0 && Array.isArray(state.tshirtPrintAreas)) {
            var _AREA_NAMES = {
                front_logo: tr('앞면 로고', '前面ロゴ', 'Front logo'),
                front_full: tr('앞면 전체', '前面全体', 'Full front'),
    ...
    // 2026-06-05: 허니콤 포토존 family — 6종 (코드 명시) + 사용자 지정 정렬 순서
    //   순서: 나무조형물 소형 → 대형 → 동화책 소형 → 대형 → 회전 큐브 → 룰렛
    var PHOTOZONE_CODES_ORDERED = [
        '45245252',     // 나무조형물 소형
        '345353',       // 나무조형물 대형
        '4354353445',   // 동화책형 포토존 소형
        '34534535',     // 동화책형 포토존 대형
        '3434535355',   // 회전하는 큐브
        '435564567'     // 허니콤보드 룰렛
    ];
    function _soIsPhotozoneProduct(p) {
        if (!p) return false;
        return PHOTOZONE_CODES_ORDERED.indexOf(p.code) >= 0;
    }
    var _soPhotozoneCache = null;
    async function _soLoadPhotozoneVariants(currentCode) {
        var sec = document.getElementById('soPhotozoneVariantsSec');
        var grid = document.getElementById('soPhotozoneVariants');
        if (!sec || !grid) return;
        try {
            if (!_soPhotozoneCache) {
                var sb = getSb(); if (!sb) { sec.style.display = 'none'; return; }
                var _COLS = 'code,name,name_jp,name_us,price,price_jp,price_us,img_url,category,sort_order,width_mm,height_mm';
                var fb = await sb.from('admin_products').select(_COLS).in('code', PHOTOZONE_CODES_ORDERED);
                var rows = (fb && fb.data) || [];
                var _byCode = {};
                rows.forEach(function(r){ if (r && r.code) _byCode[r.code] = r; });
                // PHOTOZONE_CODES_ORDERED 순서 그대로 배열 — 누락 항목은 skip
                _soPhotozoneCache = PHOTOZONE_CODES_ORDERED
                    .map(function(c){ return _byCode[c]; })
                    .filter(function(r){ return !!r; });
                console.log('[so] photozone variants loaded:', _soPhotozoneCache.length);
            }
            if (_soPhotozoneCache.length < 2) { sec.style.display = 'none'; return; }
            var lang = window.__PS_LANG || (window.__SITE_CODE === 'JP' ? 'ja' : window.__SITE_CODE === 'US' ? 'en' : 'ko');
            grid.innerHTML = _soPhotozoneCache.map(function (p) {
                var nm = p.name; if (lang === 'ja' && p.name_jp) nm = p.name_jp; else if (lang !== 'ko' && p.name_us) nm = p.name_us;
                var img = p.img_url || 'https://placehold.co/200?text=Photo';
                var safeNm = String(nm || '').replace(/[<>"]/g, '');
                var safeCode = String(p.code || '').replace(/'/g, "\\'");
                ...
                var borderW = isCur ? '2.5px' : '1.5px';
                var ring = isCur ? 'box-shadow:0 0 0 3px rgba(124,58,237,0.15);' : '';
                return '<div onclick="window._soSwitchPhotozone(\'' + safeCode + '\')" style="cursor:pointer; display:flex; flex-direction:column; border:' + borderW + ' solid ' + borderColor + '; border-radius:10px; overflow:hidden; background:#fff; transition:border-color .15s ease; ' + ring + '">' +
                    '<div style="position:relative; padding-bottom:100%; background:#f8fafc; overflow:hidden;">' +
                        '<img src="' + img + '" loading="lazy" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover;" onerror="this.style.opacity=0">' +
                        (isCur ? '<div style="position:absolute; top:4px; right:4px; background:#7c3aed; color:#fff; padding:2px 6px; border-radius:4px; font-size:9.5px; font-weight:900;">' + tr('현재','現在','Current') + '</div>' : '') +
                    '</div>' +
            ...
            sec.style.display = '';
        } catch (e) { console.warn('[so] photozoneVariants render', e); sec.style.display = 'none'; }
    }
    window._soLoadPhotozoneVariants = _soLoadPhotozoneVariants;
    window._soIsPhotozoneVariantProduct = _soIsPhotozoneProduct;

    window._soSwitchPhotozone = function (code) {
        if (!code) return;
        try {
            if (typeof window.openSimpleOrderModal === 'function') {
                window.openSimpleOrderModal(code);
    ...
    var REAL_PRINT_CODES_ORDERED = [
        '345645645',    // 1. 캘지 인쇄 실사출력
        '34534543',     // 2. 패트(PET) 인쇄
        '34554322',     // 3. 후렉스(플렉스)인쇄 UV
        '345345436',    // 4. 후렉스(플렉스)인쇄 라텍스
        '35456345345',  // 5. 매쉬천 인쇄
        '75766757',     // 6. 부직포 인쇄
        '4563435',      // 7. 인화지 인쇄
        '42355223',     // 8. 유포지 인쇄
        '456474546'     // 9. 백릿 인쇄
    ...
    // 2026-06-06: 현수막 family — 9종 (관리자 표시 순서)
    var PLACARD_CODES_ORDERED = [
        '44578',           // 1. 초저가 현수막
        '44578_copy',      // 2. UV 3미터 대폭현수막
        '45646456',        // 3. 친환경현수막 썩는원단
        '45645656',        // 4. 어깨띠
        '3453535',         // 5. 라텍스 현수막
        '5646456',         // 6. 깃발 현수막 가로등 배너
        '44578_copy_copy', // 7. 텐트천 현수막
        '34453453',        // 8. 방염현수막
        '4354536'          // 9. 족자현수막
    ];
    function _soIsPlacardProduct(p) {
        if (!p) return false;
        return PLACARD_CODES_ORDERED.indexOf(p.code) >= 0;
    }
    var _soPlacardCache = null;
    async function _soLoadPlacardVariants(currentCode) {
        var sec = document.getElementById('soPlacardVariantsSec');
        var grid = document.getElementById('soPlacardVariants');
        if (!sec || !grid) return;
        try {
            if (!_soPlacardCache) {
                var sb = getSb(); if (!sb) { sec.style.display = 'none'; return; }
                var _COLS = 'code,name,name_jp,name_us,price,price_jp,price_us,img_url,category,sort_order,width_mm,height_mm';
                var r1 = await sb.from('admin_products').select(_COLS).in('code', PLACARD_CODES_ORDERED);
                var rows = (r1 && r1.data) || [];
                var _byCode = {};
                rows.forEach(function(r){ if (r && r.code) _byCode[r.code] = r; });
                _soPlacardCache = PLACARD_CODES_ORDERED
                    .map(function(c){ return _byCode[c]; })
                    .filter(function(r){ return !!r; });
                console.log('[so] placard variants loaded:', _soPlacardCache.length);
            }
            if (_soPlacardCache.length < 2) { sec.style.display = 'none'; return; }
            var lang = window.__PS_LANG || (window.__SITE_CODE === 'JP' ? 'ja' : window.__SITE_CODE === 'US' ? 'en' : 'ko');
            grid.innerHTML = _soPlacardCache.map(function (p) {
                var nm = p.name; if (lang === 'ja' && p.name_jp) nm = p.name_jp; else if (lang !== 'ko' && p.name_us) nm = p.name_us;
                var img = p.img_url || 'https://placehold.co/200?text=Placard';
                var safeNm = String(nm || '').replace(/[<>"]/g, '');
                var safeCode = String(p.code || '').replace(/'/g, "\\'");
                var priceVal = p.price || 0;
                if (lang === 'ja' && p.price_jp != null) priceVal = p.price_jp;
                ...
                var borderW = isCur ? '2.5px' : '1.5px';
                var ring = isCur ? 'box-shadow:0 0 0 3px rgba(124,58,237,0.15);' : '';
                return '<div onclick="window._soSwitchPlacard(\'' + safeCode + '\')" style="cursor:pointer; display:flex; flex-direction:column; border:' + borderW + ' solid ' + borderColor + '; border-radius:10px; overflow:hidden; background:#fff; transition:border-color .15s ease; ' + ring + '">' +
                    '<div style="position:relative; padding-bottom:100%; background:#f8fafc; overflow:hidden;">' +
                        '<img src="' + img + '" loading="lazy" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover;" onerror="this.style.opacity=0">' +
                        (isCur ? '<div style="position:absolute; top:4px; right:4px; background:#7c3aed; color:#fff; padding:2px 6px; border-radius:4px; font-size:9.5px; font-weight:900;">' + tr('현재','現在','Current') + '</div>' : '') +
                    '</div>' +
            ...
            sec.style.display = '';
        } catch (e) { console.warn('[so] placardVariants render', e); sec.style.display = 'none'; }
    }
    ...
    function _soIsLeafletProduct(p) {
        if (!p) return false;
        var code = (p.code || '').toLowerCase();
        if (code.startsWith('pp_lf')) return true;
    ...
    function _soIsVinylProduct(p) {
        if (!p) return false;
        return VINYL_CODES_ORDERED.indexOf(p.code) >= 0;
    }
    var _soVinylCache = null;
    async function _soLoadVinylVariants(currentCode) {
        var sec = document.getElementById('soVinylVariantsSec');
        var grid = document.getElementById('soVinylVariants');
        if (!sec || !grid) return;
        try {
            if (!_soVinylCache) {
                var sb = getSb(); if (!sb) { sec.style.display = 'none'; return; }
                var _COLS = 'code,name,name_jp,name_us,price,price_jp,price_us,img_url,category,sort_order,width_mm,height_mm';
                var r1 = await sb.from('admin_products').select(_COLS).in('code', VINYL_CODES_ORDERED);
                var rows = (r1 && r1.data) || [];
                var _byCode = {};
                rows.forEach(function(r){ if (r && r.code) _byCode[r.code] = r; });
                _soVinylCache = VINYL_CODES_ORDERED
                    .map(function(c){ return _byCode[c]; })
                    .filter(function(r){ return !!r; });
                console.log('[so] vinyl variants loaded:', _soVinylCache.length);
            }
            if (_soVinylCache.length < 2) { sec.style.display = 'none'; return; }
            var lang = window.__PS_LANG || (window.__SITE_CODE === 'JP' ? 'ja' : window.__SITE_CODE === 'US' ? 'en' : 'ko');
            grid.innerHTML = _soVinylCache.map(function (p) {
                var nm = p.name; if (lang === 'ja' && p.name_jp) nm = p.name_jp; else if (lang !== 'ko' && p.name_us) nm = p.name_us;
                var img = p.img_url || 'https://placehold.co/200?text=Vinyl';
                var safeNm = String(nm || '').replace(/[<>"]/g, '');
                var safeCode = String(p.code || '').replace(/'/g, "\\'");
                var priceVal = p.price || 0;
                if (lang === 'ja' && p.price_jp != null) priceVal = p.price_jp;
                ...
                var borderW = isCur ? '2.5px' : '1.5px';
                var ring = isCur ? 'box-shadow:0 0 0 3px rgba(124,58,237,0.15);' : '';
                return '<div onclick="window._soSwitchVinyl(\'' + safeCode + '\')" style="cursor:pointer; display:flex; flex-direction:column; border:' + borderW + ' solid ' + borderColor + '; border-radius:10px; overflow:hidden; background:#fff; transition:border-color .15s ease; ' + ring + '">' +
                    '<div style="position:relative; padding-bottom:100%; background:#f8fafc; overflow:hidden;">' +
                        '<img src="' + img + '" loading="lazy" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover;" onerror="this.style.opacity=0">' +
                        (isCur ? '<div style="position:absolute; top:4px; right:4px; background:#7c3aed; color:#fff; padding:2px 6px; border-radius:4px; font-size:9.5px; font-weight:900;">' + tr('현재','現在','Current') + '</div>' : '') +
                    '</div>' +
            ...
            sec.style.display = '';
        } catch (e) { console.warn('[so] vinylVariants render', e); sec.style.display = 'none'; }
    }
    ...
    window._soSwitchVinyl = function (code) {
        ...
        } catch (e) { console.warn('[so] switchVinyl', e); }
    };
    ...
    var BASE_STAND_OPTS = {
        ...
        // 2026-06-04: 자유인쇄커팅 (hb_pt_*) — 받침 종류별 유료. 등신대(hb_pi_5)는 무료 유지 (별도 override).
    ...
    async function _soPopulateAddons(p) {
        ...
        // 2026-06-03: 명함 (pp_bc_*) — 자체 용지/박/후가공 UI 사용. 기존 admin_addons 표시 차단.
        if (_soIsBizCardProduct(p)) return;
        // 2026-06-03: 스티커 — 자체 카테고리/사이즈/용지/수량 UI 사용. 기존 admin_addons 차단.
        if (_soIsStickerProduct(p)) return;
        ...
        var html = renderList.map(function (a) {
            ...
            return '<label style="display:flex; align-items:center; gap:10px; padding:10px 12px; border:2px solid #e7e5e4; border-radius:12px; cursor:pointer; font-size:13px; background:#fff; transition:all 0.15s;">' +
                ...
                '</div>' +
                // 2026-05-22: 옵션 수량 직접 입력 (한 대지에 여러 개 들어가는 경우). 수량 입력 시 자동 체크.
                '<input type="number" min="1" value="1" data-addon-qty-code="' + String(a.code).replace(/"/g,'&quot;') + '" onclick="event.stopPropagation();" oninput="window._soSetAddonQty(this)" title="수량" style="width:50px; padding:5px 4px; border:1px solid #d1d5db; border-radius:6px; text-align:center; font-size:13px; font-weight:700; flex-shrink:0;">' +
    ...
    // 2026-05-30: 티셔츠 — 새 통합 목업 (앞면+뒷면 가로 한 장) + 드래그 가능한 인쇄 영역 박스
    ...
    window._soRenderTshirtUploads = function () {
        ...
        // 통합 목업 (앞면 좌, 뒷면 우). t/tshirt_mockup.jpg 가 없으면 기존 print1/2/3 로 fallback.
        var MOCKUP = '/t/tshirt_mockup.jpg';
        // side = 'front' (목업 좌측 표시) | 'back' (목업 우측 표시)
        // bgPos = background-position-x 값 (목업 좌측만 보이게 0%, 우측만 보이게 100%)
        // initBox = 초기 박스 위치/크기 (% 기준, 박스 좌상단 기준)
        var CFG = {
            front_logo: { side:'front', bgPos:'0%',   ko:'앞면 로고', jp:'前面ロゴ', en:'Front logo',
                          init:{ left:38, top:28, width:18, height:18 } }, // 로고 (정사각, 가슴 중앙)
            front_full: { side:'front', bgPos:'0%',   ko:'앞면 전체', jp:'前面全体', en:'Full front',
                          init:{ left:25, top:22, width:42, height:60 } }, // A4 비례 (42:60 ≈ 210:297)
            back_full:  { side:'back',  bgPos:'100%', ko:'뒷면 전체', jp:'背面全体', en:'Full back',
                          init:{ left:25, top:22, width:42, height:60 } }
        };
    ...
    // 드래그 + 클릭 부착
    function _attachTshirtBoxDrag(box) {
        var dragging = false;
        var moved = false;
        ...
        box.addEventListener('pointermove', function(e){
            ...
            var widthPct = parseFloat(box.style.width);
            var heightPct = parseFloat(box.style.height);
            var newLeft = startLeft + (dx / w) * 100;
            var newTop  = startTop  + (dy / h) * 100;
            newLeft = Math.max(0, Math.min(100 - widthPct, newLeft));
            newTop  = Math.max(0, Math.min(100 - heightPct, newTop));
            box.style.left = newLeft + '%';
            box.style.top  = newTop + '%';
            if (state.tshirtFiles && state.tshirtFiles[area] && state.tshirtFiles[area].box) {
                state.tshirtFiles[area].box.left = newLeft;
                state.tshirtFiles[area].box.top  = newTop;
            }
        });
        box.addEventListener('pointerup', function(e){
            ...
            // 단순 클릭(드래그 거의 없음) 이면 파일 선택창 열기
            if (!moved) {
                var input = document.getElementById('soTshirtFile_' + area);
    ...
    window._soTshirtPickFile = function (area, files) {
        ...
        if (!state.tshirtFiles) state.tshirtFiles = {};
        // 기존 박스 위치 보존
        var prevBox = (state.tshirtFiles[area] && state.tshirtFiles[area].box) || null;
    ...
    // 2026-06-08: 실사출력 — 원단 폭 선택 (90 / 127). 127폭은 단가 × 1.2.
    window._soPickRealPrintWidth = function (btn) {
        if (!btn) return;
        document.querySelectorAll('.so-rp-width-btn').forEach(function(b){
            b.classList.remove('active');
            b.style.background = '#fff';
            b.style.color = '#334155';
        ...
        state.realPrintWidth = btn.getAttribute('data-width') === '127' ? '127' : '90';
        if (typeof recalc === 'function') recalc();
    };
    // 2026-06-08: 실사출력 (캘지/유포지 한정) — 표면 코팅 선택. UV 무료 / 무광 +1K/m / 유광 +2K/m.
    window._soPickRealPrintCoating = function (btn) {
        if (!btn) return;
        document.querySelectorAll('.so-rp-coat-btn').forEach(function(b){
            b.classList.remove('active');
            b.style.background = '#fff';
            b.style.color = '#334155';
            b.style.border = '2px solid #e2e8f0';
    ...
    // 2026-06-04: 등신대/cutPrint 4-card UI 는 mutex (1개만 선택, 라디오 동작)
    ...
    function _soBizCardRender() {
        ...
        if (tg && tp) {
            ...
            tg.style.background = prem ? '#fff' : 'linear-gradient(135deg,#ec4899,#be185d)';
            tg.style.color      = prem ? '#451a03' : '#fff';
            tg.style.borderColor= prem ? '#e7e5e4' : '#be185d';
            tp.style.background = prem ? 'linear-gradient(135deg,#ec4899,#be185d)' : '#fff';
            tp.style.color      = prem ? '#fff' : '#451a03';
            tp.style.borderColor= prem ? '#be185d' : '#e7e5e4';
        }
        // 인쇄면 버튼 — 활성 시 인디고 그라데이션 (등급 핑크와 구분)
        ...
        if (bs && bd) {
            ...
            bs.style.background = dbl ? '#fff' : 'linear-gradient(135deg,#6366f1,#4338ca)';
            bs.style.color = dbl ? '#451a03' : '#fff';
            bs.style.borderColor = dbl ? '#e7e5e4' : '#4338ca';
            bd.style.background = dbl ? 'linear-gradient(135deg,#6366f1,#4338ca)' : '#fff';
    ...
    // 2026-06-13: 칼선작업 체크박스 — 한 종류씩 카트 담기이므로 개수 입력 제거. 체크되면 항상 +10K (라인 1건당).
    window._soToggleCutline = function(checked) {
        state.cutlineWork = !!checked;
        state.cutlineCharCount = checked ? 1 : 0;
    ...
    window._soRefreshCutlineUI = function() {
        ...
        if (state.cutlineWork) state.cutlineCharCount = 1;
    ...
    window._soToggleAddon = function (inp) {
        ...
        if (inp.checked) {
            ...
            } else if (state.isAcrylicGoods || state.isStandee) {
                state.addonQuantities[code] = state.qty || 1;
            } else {
                state.addonQuantities[code] = (_qInp && parseInt(_qInp.value, 10) > 0) ? parseInt(_qInp.value, 10) : 1;
            }
            if (_qInp) _qInp.value = state.addonQuantities[code];
        } else {
    ...
    // 2026-05-22: 옵션 수량 직접 입력 — 한 대지에 여러 개 들어가는 경우 (자유인쇄커팅 등)
    window._soSetAddonQty = function (inp) {
        var code = inp.dataset.addonQtyCode;
        var q = Math.max(1, parseInt(inp.value, 10) || 1);
        inp.value = q;
        if (!state.selectedAddons) state.selectedAddons = {};
        if (!state.addonQuantities) state.addonQuantities = {};
        // 수량을 입력하면 해당 옵션 자동 체크
        var lbl = inp.closest('label');
        var cb = lbl ? lbl.querySelector('input[type=checkbox][data-addon-code]') : null;
        if (cb && !cb.checked) cb.checked = true;
        state.selectedAddons[code] = code;
        state.addonQuantities[code] = q;
        recalc();
    };
    ...
    window.openSimpleOrderModal = async function(productCode, productData) {
        ...
        // 2026-06-03: 스티커 (st_*, 0000241) — 종합 스티커 페이지 (12 카테고리)
        state.isSticker = _soIsStickerProduct(p);
        ...
        // 2026-05-25: hexa-board.com(화이트라벨)에서 열린 상품은 모두 원판으로 처리 (카테고리 매핑 미로드 대비)
        ...
        // 2026-06-03: 인쇄물 카테고리(모든 일반 인쇄물) + 스티커 — 수량할인 tier 표 숨김 (사용자 요청)
        //   isGeneralPrint = 허니콤/포맥스 제외 = 광고/상업/홈/굿즈/기타 모든 인쇄물
        if (_rb_tier && (_soIsGeneralPrintProduct(p) || _soIsBizCardProduct(p) || _soIsStickerProduct(p))) {
        ...
        // 2026-06-04: 등신대 + 자유인쇄커팅 — 받침대 UI 를 4-card 로 교체. 가격은 상품별로 다름:
        //   등신대(hb_pi_5): 모두 무료 (안내 배너 + FREE 라벨)
        //   자유인쇄커팅(hb_pt_*): 끼우는3k / 뒷면8k / 종이500원 / 받침없음 무료 (가격 표시)
        ...
        //   배송비 10만원 이하 1만원 / 이상 무료 (카트 합계는 _soCalcCartTotal 에서 별도 batch 계산).
        state.isRealPrint = !!(typeof window._soIsRealPrintProduct === 'function' && window._soIsRealPrintProduct(p));
        if (state.isRealPrint) {
            state.isAdPrint = false;        // ad-print layout 비활성
            state.isCustomSize = false;     // 사이즈 W×H 계산기 비활성
            state.realPrintWidth = '90';    // 90폭 default
            state.realPrintCoating = 'uv';  // UV (무료) default
            // 시공/배송 옵션 섹션 숨김 — _soComputeShipFee 가 자동 처리
            try {
                var _rpSchedSec = document.getElementById('soScheduleSection');
                if (_rpSchedSec) _rpSchedSec.style.display = 'none';
            } catch (e) {}
        }
        ...
        (function _soApplyAdPrintLayout(){
            ...
            // 2026-06-08: 실사출력 family 전용 layout — width/coating 섹션 표시 + customSize/multi-line 숨김.
            ...
            if (state.isRealPrint && _rpSec) {
                ...
                // 시공/배송 옵션 섹션 숨김
                if (_schedSec) _schedSec.style.display = 'none';
                // 좌측 업로드는 표시 (디자인 파일 첨부용)
                if (_leftUpload) _leftUpload.style.display = '';
                if (_leftUploadLabel) _leftUploadLabel.style.display = '';
                if (_inlineUploadCard) _inlineUploadCard.style.display = 'none';  // 인라인 업로드 카드는 ad-print 전용
                // 코팅 선택은 캘지(345645645) / 유포지(42355223) 만
                var _rpCode = (p && p.code) || '';
                if (_rpCoatWrap) _rpCoatWrap.style.display = (_rpCode === '345645645' || _rpCode === '42355223') ? '' : 'none';
                // 폭/코팅 active 버튼 초기화 (90 / uv)
                document.querySelectorAll('.so-rp-width-btn').forEach(function(b){
                    var _on = b.getAttribute('data-width') === '90';
                    b.classList.toggle('active', _on);
                    b.style.background = _on ? '#0f172a' : '#fff';
                    b.style.color      = _on ? '#fff'    : '#334155';
                    b.style.border     = '2px solid ' + (_on ? '#0f172a' : '#e2e8f0');
                });
                document.querySelectorAll('.so-rp-coat-btn').forEach(function(b){
                    var _on = b.getAttribute('data-coat') === 'uv';
                    b.classList.toggle('active', _on);
                    b.style.background = _on ? '#0f172a' : '#fff';
                    b.style.color      = _on ? '#fff'    : '#334155';
                    b.style.border     = '2px solid ' + (_on ? '#0f172a' : '#e2e8f0');
                ...
                // qty 입력 input min=1
                var _rpQtyInp = document.getElementById('soQty');
                ...
                if (qtySec) {
                    ...
                    if (_qtyUnit2) _qtyUnit2.textContent = tr('개', '個', 'pcs');
                }
            }
    ...
    function buildCartItem(fileUrl, filePath) {
        ...
        return {
            ...
            // 2026-06-08: 실사출력 family — 1미터 단위, 폭(90/127) + 코팅(uv/matte/gloss) 트리거. 30m+ 30% 자동.
            _isRealPrint: !!state.isRealPrint,
            realPrintWidth: state.isRealPrint ? (state.realPrintWidth || '90') : null,
            realPrintCoating: state.isRealPrint ? (state.realPrintCoating || 'uv') : null,
        ...
        try {
            ...
            if ((state.isAdPrint || state.isHoneycomb || state.isCutPrint) && Array.isArray(state._adLines) && state._adLines.length > 0) {
                ...
                for (var _li = 0; _li < state._adLines.length; _li++) {
                    ...
                    // 2026-06-01: 현재 라인 push 가 skip 됐다면 첫 번째 큐 라인이 배송비 캐리어 (수도권 설치 등)
                    //   _skipCurrent && _li===0 → bundleShipping=false (배송비 보존). 나머지는 묶음.
                    state.bundleShipping = (_skipCurrent && _li === 0) ? false : true;
                    // 모드별 추가 state 복원
    ...
    function calcCartItemPrice(item) {
        ...
        // 2026-06-03: 명함/리플렛 — 정액 가격, 수량 할인 없음
    ...
    function renderSoCart() {
        ...
        // 2026-05-30: 매니저 견적 주문 항목 자동 정리 — 일반 카트 드로어에 노출 X (전용 결제창 흐름에서만 사용)
        //   서버 cart_sync 가 끌어와도 화면에는 안 보이며, 다음 push 때 자동 제거됨
        const _isMgrQuoteItem = function (it) {
            if (!it) return false;
        ...
        var _beforeFilter = allItems.length;
        allItems = allItems.filter(function(it){ return _isValidItem(it) && !_isMgrQuoteItem(it); });
        var _purgedCount = _beforeFilter - allItems.length;
        if (_purgedCount > 0) {
            console.log('[so cart] purged ' + _purgedCount + ' manager_quote item(s) from cart view');
            // localStorage / cart_sync 에서도 영구 제거 (다음 페이지에서 또 안 보이도록)
            try {
                if (window.cartSync && typeof window.cartSync.forceSync === 'function') {
                    var raw = JSON.parse(localStorage.getItem(CART_KEY) || '[]');
                    var cleaned = raw.filter(function(it){ return !_isMgrQuoteItem(it); });
                    if (cleaned.length !== raw.length) {
                        localStorage.setItem(CART_KEY, JSON.stringify(cleaned));
                        window.cartSync.forceSync();
                    }
                }
            } catch (e) { console.warn('[so cart] mgr quote cleanup', e); }
        }
        ...
        if (generalItems.length > 0) {
            const genHtml = generalItems.map((item) => {
                ...
                // 단가는 원본 제품가 우선 (calc.unit 은 옵션/배송 등 합산값일 수 있어 사용 X)
                var _bdUnit = (item.product && item.product.price) || calc.unit || 0;
    ...
    function _soCalcItemPrice(it) {
        ...
        // 2026-06-08: 실사출력 family — 1미터 단위. 단가 = product.price × (90폭 1.0 / 127폭 1.2) + 코팅비(0/1K/2K).
        //   30m+ 30% 자동 할인. 배송비는 _soCalcCartTotal 의 batch 룰로 별도 처리 (이 함수에서는 skip).
        ...
        // 2026-06-08: 실사출력 family 도 무료배송 — 배송비 합산 skip
        var _isGoodsItm = it.product && it.product.code && it.product.code.indexOf('goods_') === 0;
        if (it.bundleShipping || _isGoodsItm || _isRealPrintItm) {
            // skip — 배송비 0
        } else if (_isBest) {
    ...
    function _soCalcCartTotal(cart) {
        ...
        // 2026-06-08: 1원단위/천원단위 금액주문 (21355677*) + 매니저 견적 (manager_quote_*) 은 배송비 자동 가산 제외.
        //   입력 금액 그대로 결제 (별도 배송비/할인 없음).
        ...
        var _hasAmountOrder = cart.some(function(_it){
            ...
            if (_c.indexOf('manager_quote_') === 0) return true;
    ...
    window._soInitWallet = async function () {
        ...
        window._soWallet = {
            ...
            mileageBalKRW: mileageBal, depositBalKRW: depositBal, eventCouponBalKRW: eventCouponBal,
            eventMax: eventMax, mileageMax: mileageMax, depositMax: depositMax, proMax: proMax,
            // legacy 호환: capKRW = 가장 큰 사용 가능값 (라디오로 선택된 것에 따라 동적 변경)
            capKRW: 0, discChoice: null, excluded: excluded
        };
        // 각 카드 표시 갱신
        var setCard = function(disc, amtId, hintId, balance, maxApply, label, suffix){
            ...
            if (card) {
                ...
                card.style.cursor = disable ? 'not-allowed' : 'pointer';
            }
            if (radio) radio.disabled = disable;
            if (amtEl) amtEl.textContent = disable ? '-' : ('-' + maxApply.toLocaleString() + suffix);
    ...
    // 2026-06-01: 4박스 라디오 mutex — 선택된 1개의 할인만 사용.
    //   _soWallet.discChoice ∈ { null, 'event_coupon', 'mileage', 'deposit', 'pro' }
    //   _soGetWalletUseKRW 는 선택값에 따라 useMileage / useDeposit / proSuppressed 산출.
    //   - 'event_coupon' : useMileage = eventMax, 잔액은 profiles.event_coupon 에서 차감 (소스 표시)
    //   - 'mileage'      : useMileage = mileageMax (legacy 5%), 잔액은 profiles.mileage 에서 차감
    ...
    function _soApplyWalletToTotal() {
        ...
        if (bd) {
            var html = '';
            if (w.source === 'event_coupon' && w.useMileage > 0) {
                html += '<div style="display:flex; justify-content:space-between; color:#b45309;"><span>· 🎁 ' + tr('이벤트 쿠폰 사용','イベントクーポン使用','Event coupon') + '</span><span>-' + _soFormatPrice(w.useMileage) + '</span></div>';
            }
            if (w.source === 'mileage' && w.useMileage > 0) {
                html += '<div style="display:flex; justify-content:space-between; color:#4338ca;"><span>· ⭐ ' + tr('마일리지 사용','マイル使用','Mileage') + '</span><span>-' + _soFormatPrice(w.useMileage) + '</span></div>';
            }
    ...
    // 2026-06-01: legacy handlers — 4박스 라디오로 대체됨. no-op 으로 유지 (외부 호출 호환).
    ...
    window._soSubmitOrder = async function () {
        ...
        try {
            ...
            cart.forEach(function (it, idx) {
                ...
                if (it._presetType === 'tshirt') {
                    ...
                    if (_areas && _areas.length) {
                        var _FEE_NOTE = { front_logo: 3000, front_full: 8000, back_full: 8000 };
                        var _areaNames = _areas.map(function(a){
                            var fee = _FEE_NOTE[a] || 0;
            ...
            //   - source='mileage'      → profiles.mileage 에서 차감 (legacy)
            ...
            if (_walletUid && (_useMileage > 0 || _useDeposit > 0)) {
                try {
                    if (_useMileage > 0 && _walletSource === 'event_coupon') {
                        // 이벤트 쿠폰 컬럼에서 차감
                        var _ecRow = null;
                        try { var _ecR = await sb.from('profiles').select('event_coupon').eq('id', _walletUid).maybeSingle(); _ecRow = _ecR.data; } catch(e) {}
                        var _ce = parseInt((_ecRow && _ecRow.event_coupon) || 0) || 0;
                        if (_ecRow && 'event_coupon' in _ecRow) {
                            await sb.from('profiles').update({ event_coupon: Math.max(0, _ce - _useMileage) }).eq('id', _walletUid);
                        } else {
                            // 컬럼 미적용 환경 — mileage 에서 fallback 차감
