...
window._mqCreateQuote = async function (btn) {
    ...
    try {
        ...
        btn.innerHTML = '✅ 생성 완료';
        // 주문 목록 새로고침 (관리자가 곧바로 확인할 수 있도록)
        if (typeof window.loadOrders === 'function') { try { window.loadOrders(); } catch (e) {} }
    } catch (e) {
        console.error('[manager_quote] create failed:', e);
        alert('결제창 생성 오류: ' + (e.message || e));
        btn.disabled = false;
        btn.innerHTML = '⚡ 결제창 생성하기';
    }
};
