...
import { canvas } from "./canvas-core.js?v=348";
import { PRODUCT_DB, ADDON_DB, ADDON_CAT_DB, cartData, currentUser, sb } from "./config.js?v=348";
import { SITE_CONFIG } from "./site-config.js?v=348";
import { applySize } from "./canvas-size.js?v=348";
import { pageDataList, currentPageIndex } from "./canvas-pages.js?v=348";
import {
...
async function addCanvasToCart() {
    ...
    if (window.__boxMode && window.__boxNesting && window.__boxDims) {
        try {
            const { generateBoxLayoutPDF } = await import('./export.js?v=348');
    ...
    for (let i = 0; i < cartData.length; i++) {
        ...
        if (!item.originalUrl && item.type === 'design' && item.json && item.product) {
            ...
            try {
                ...
                const { generateDesignPNG } = await import('./export.js?v=348');
