...
import { canvas } from "./canvas-core.js?v=434";
import { PRODUCT_DB, ADDON_DB, ADDON_CAT_DB, cartData, currentUser, sb } from "./config.js?v=434";
import { SITE_CONFIG } from "./site-config.js?v=434";
import { applySize } from "./canvas-size.js?v=434";
import { pageDataList, currentPageIndex } from "./canvas-pages.js?v=434";
import {
        ...
        btnGoCheckout.onclick = () => {
            ...
            if (_cartTotal < 1000000 && !_shipExempt) {
                ...
                if (!_hasShip) {
                    ...
                    if (_box) { _box.scrollIntoView({behavior:'smooth', block:'center'}); _box.style.outline='3px solid #ef4444'; _box.style.borderRadius='10px'; setTimeout(()=>{_box.style.outline='';}, 2500); }
                    return;
                }
            }
...
async function addCanvasToCart() {
    ...
    if (window.__boxMode && window.__boxNesting && window.__boxDims) {
        try {
            const { generateBoxLayoutPDF } = await import('./export.js?v=434');
                    ...
                    <div style="flex:1; min-width:200px;">
                        ...
                        <div style="margin-top:8px; display:flex; gap:6px; align-items:center; flex-wrap:wrap;">
                            ${item.originalUrl
                                ...
                                : `<label style="cursor:pointer; font-size:12px; color:#fff; background:#dc2626; padding:6px 14px; border-radius:8px; font-weight:700; display:inline-flex; align-items:center; gap:4px;"><i class="fa-solid fa-paperclip"></i> ${(window.t && window.t('cart_file_attach')) || '파일 첨부'}<input type="file" style="display:none;" onchange="window._uploadCartItemFile(${idx}, this)"></label>`}
                        </div>
...
async function processOrderSubmission() {
    ...
    if (!isExempt && rawTotal < MIN_ORDER_KRW) {
        ...
        const line1 = { kr: '업자만을 위한 도매 쇼핑몰입니다.', ja: '業者専用の卸売ショッピングモールです。', en: 'A wholesale shop exclusively for businesses.', zh: '仅供业者使用的批发商城。', ar: 'متجر جملة حصري للشركات.', es: 'Tienda mayorista exclusiva para profesionales.', de: 'Großhandelsshop ausschließlich für Geschäftskunden.', fr: 'Boutique de gros réservée aux professionnels.' };
        ...
        const line3 = { kr: '최소 주문금액이 있는만큼 품질과 가격으로 보답하겠습니다. 소액 주문량이 너무 폭주하여 최소 주문금액을 설정하였습니다. 양해 부탁드립니다.', ja: '最低注文金額がある分、品質と価格でお応えします。小額注文の急増により最低金額を設定しました。ご了承ください。', en: 'With our minimum order, we deliver superior quality and best prices. Set due to overwhelming small orders. Thank you for understanding.', zh: '设有最低订购金额，以品质和价格回报您。因小额订单激增故设置最低金额，敬请谅解。', ar: 'مع الحد الأدنى للطلب، نضمن جودة وأسعارًا ممتازة. تم تعيينه بسبب تزايد الطلبات الصغيرة. شكرًا لتفهمكم.', es: 'Con nuestro pedido mínimo, garantizamos calidad y mejor precio. Establecido por la avalancha de pedidos pequeños. Gracias por su comprensión.', de: 'Mit unserer Mindestbestellung garantieren wir Qualität und Bestpreise. Aufgrund vieler Kleinbestellungen eingeführt. Danke für Ihr Verständnis.', fr: 'Grâce à notre commande minimum, nous garantissons qualité et prix imbattables. Mis en place suite à l\'afflux de petites commandes. Merci de votre compréhension.' };
    ...
    for (let i = 0; i < cartData.length; i++) {
        ...
        if ((!item.originalUrl || _hasMultiPages) && item.type === 'design' && (item.json || _hasMultiPages) && item.product) {
            ...
            try {
                ...
                const { generateDesignPNG } = await import('./export.js?v=434');
