...
import { PRODUCT_DB, ADDON_DB, ADDON_CAT_DB, cartData, currentUser, sb } from "./config.js?v=163";
import { SITE_CONFIG } from "./site-config.js?v=163";
import { applySize } from "./canvas-size.js?v=163";
import { pageDataList, currentPageIndex } from "./canvas-pages.js?v=163";
import {
    ...
    for (let i = 0; i < cartData.length; i++) {
        ...
        if (!item.originalUrl && item.type === 'design' && item.json && item.product) {
            ...
            try {
                ...
                const { generateDesignPNG } = await import('./export.js?v=163');
