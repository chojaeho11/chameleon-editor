...
import { PRODUCT_DB, ADDON_DB, ADDON_CAT_DB, cartData, currentUser, sb } from "./config.js?v=291";
import { SITE_CONFIG } from "./site-config.js?v=291";
import { applySize } from "./canvas-size.js?v=291";
import { pageDataList, currentPageIndex } from "./canvas-pages.js?v=291";
import {
...
async function addCanvasToCart() {
    ...
    if (window.__boxMode && window.__boxNesting && window.__boxDims) {
        try {
            const { generateBoxLayoutPDF } = await import('./export.js?v=291');
                ...
                <div style="display:flex; width:100%; padding:20px; gap:30px; align-items:flex-start;">
                    ...
                    <div style="flex:1; min-width:200px;">
                        ...
                        <div style="margin-top:8px; display:flex; gap:6px; align-items:center; flex-wrap:wrap;">
                            ${item.originalUrl ? `<a href="${item.originalUrl}" target="_blank" style="font-size:11px; color:#059669; text-decoration:none; background:#ecfdf5; padding:3px 10px; border-radius:5px; border:1px solid #a7f3d0;"><i class="fa-solid fa-check-circle"></i> ${item.fileName || '파일 첨부됨'}</a>` : `<label style="cursor:pointer; font-size:12px; color:#fff; background:#dc2626; padding:6px 14px; border-radius:8px; font-weight:700; display:inline-flex; align-items:center; gap:4px;"><i class="fa-solid fa-paperclip"></i> 파일 첨부<input type="file" style="display:none;" onchange="window._uploadCartItemFile(${idx}, this)"></label>`}
                            ${(item.extraFiles && item.extraFiles.length > 0) ? item.extraFiles.map((f, fi) => `<a href="${f.url}" target="_blank" style="font-size:10px; color:#059669; background:#ecfdf5; padding:2px 8px; border-radius:4px; border:1px solid #a7f3d0;"><i class="fa-solid fa-check"></i> ${fi+2}</a>`).join('') : ''}
                            ${(!item.originalUrl && item.qty > 1) ? `<span style="font-size:10px; color:#94a3b8;">* 디자인이 여러개면 첨부 후 추가 파일 업로드 가능</span>` : ''}
                        </div>
    ...
    for (let i = 0; i < cartData.length; i++) {
        ...
        if (!item.originalUrl && item.type === 'design' && item.json && item.product) {
            ...
            try {
                ...
                const { generateDesignPNG } = await import('./export.js?v=291');
