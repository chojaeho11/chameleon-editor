...
window.openAdminSlotModal = async (dateStr) => {
    ...
    try {
        ...
        const _isBoardCourier   = (fee) => fee === 30000;     // 보드류 택배
        const _isStdCourier     = (fee) => fee === 5000;      // 기타제품 일반택배
        const _isMetroPaidInst  = (fee) => fee === 100000;    // 수도권 유료설치
        const _isLocalTruck     = (fee) => fee === 200000;    // 지방 용차배송
        ...
        // ===== 좌측: 3 시간대 × 3팀 매트릭스 =====
        // 2026-08-16: 성수기 캐파 오전4/오후4/야간3/시간무관12 (order.js PERIOD_CAPACITY 와 동기화)
        const PERIOD_META = {
            am:    { label:'🌅 오전',  sub:'08:00–12:00', cap:4,  bg:'#fef3c7', fg:'#92400e' },
            pm:    { label:'☀️ 오후',  sub:'12:00–18:00', cap:4,  bg:'#fed7aa', fg:'#9a3412' },
