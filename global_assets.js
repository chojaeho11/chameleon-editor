...
// ==========================================
// 썸네일 마이그레이션 (base64 → Supabase Storage)
// ==========================================

// 이미지를 지정 크기로 리사이즈하여 JPEG Blob 반환
function _resizeToBlob(src, maxSize) {
...
// data_url (Fabric.js JSON) 에서 고해상도 이미지 src 추출
function _extractImageFromFabricJson(jsonStr) {
    try {
        const json = JSON.parse(jsonStr);
        const objs = json.objects || [];
        // 이미지 타입 오브젝트 중 가장 큰 것 (배경 이미지일 가능성)
        let bestSrc = null;
        for (const o of objs) {
            if (o.type === 'image' && o.src && o.src.startsWith('data:image')) {
                bestSrc = o.src;
                break; // 첫 번째 이미지 = 보통 배경
            }
        }
        return bestSrc;
    } catch (e) {
...
window._thumbMigrationStop = false;

window.migrateBase64Thumbs = async () => {
    const statusEl = document.getElementById('thumbMigrationStatus');
    const btnStart = document.getElementById('btnThumbMigrate');
    const btnStop = document.getElementById('btnThumbMigrateStop');
    if (!statusEl) return;
    ...
    // 총 개수 확인
    const { count } = await sb.from('library')
        .select('id', { count: 'exact', head: true })
        .like('thumb_url', 'data:%');

    if (!count || count === 0) {
        statusEl.innerHTML = '<span style="color:#16a34a;">✅ 마이그레이션 할 항목이 없습니다. (모두 완료)</span>';
        if (btnStart) btnStart.disabled = false;
        if (btnStop) btnStop.style.display = 'none';
        return;
    }

    statusEl.innerHTML = `🔄 총 <b>${count.toLocaleString()}</b>개 항목 처리 시작...`;

    let processed = 0, failed = 0, skipped = 0;
    const BATCH = 10;

    while (!window._thumbMigrationStop) {
        // ID만 가져와서 메모리 절약
        const { data: ids } = await sb.from('library')
            .select('id')
            .like('thumb_url', 'data:%')
            .limit(BATCH);
        ...
        for (const { id } of ids) {
            if (window._thumbMigrationStop) break;

            try {
                // 개별 레코드 로드 (data_url 포함)
                const { data: rec } = await sb.from('library')
                    .select('id, thumb_url, data_url, category')
                    .eq('id', id)
                    .single();

                if (!rec || !rec.thumb_url || !rec.thumb_url.startsWith('data:')) {
                    skipped++; continue;
                }

                // 1. 고해상도 이미지 소스 결정
                let imgSrc = rec.thumb_url; // 폴백: 기존 base64 썸네일 그대로

                if (rec.data_url) {
                    if (rec.data_url.trimStart().startsWith('{')) {
                        // Fabric.js JSON → 내부 이미지 추출
                        const hiRes = _extractImageFromFabricJson(rec.data_url);
                        if (hiRes) imgSrc = hiRes;
                    } else if (rec.data_url.startsWith('data:image')) {
                        // base64 이미지 직접
                        imgSrc = rec.data_url;
                    }
                    // data_url이 http URL인 경우는 img로 로드 가능
                    else if (rec.data_url.startsWith('http') && !rec.data_url.endsWith('.json') && !rec.data_url.endsWith('.svg')) {
                        imgSrc = rec.data_url;
                    }
                }

                // 2. 800px로 리사이즈
                const blob = await _resizeToBlob(imgSrc, 800);
                if (!blob) { failed++; continue; }

                // 3. Supabase Storage 업로드
                const path = `thumbs/m_${rec.id}.jpg`;
                const { error: upErr } = await sb.storage.from('design').upload(path, blob, {
                    contentType: 'image/jpeg',
                    upsert: true
                });
                if (upErr) { failed++; console.error(`Upload fail ${rec.id}:`, upErr.message); continue; }

                // 4. Public URL 얻기
                const { data: urlData } = sb.storage.from('design').getPublicUrl(path);
                if (!urlData?.publicUrl) { failed++; continue; }

                // 5. DB 업데이트
                const { error: dbErr } = await sb.from('library')
                    .update({ thumb_url: urlData.publicUrl })
                    .eq('id', rec.id);
                if (dbErr) { failed++; console.error(`DB fail ${rec.id}:`, dbErr.message); continue; }

                processed++;
            } catch (e) {
                failed++;
                console.error(`Error record ${id}:`, e);
            }

            // 진행률 업데이트
            const total = processed + failed + skipped;
            const pct = Math.round(total / count * 100);
            statusEl.innerHTML = `🔄 처리 중... <b>${processed.toLocaleString()}</b> / ${count.toLocaleString()} 완료 (${pct}%) | 실패: ${failed} | 스킵: ${skipped}`;
        }
    }

    const msg = window._thumbMigrationStop
        ? `⏸️ 중지됨 — ${processed}개 완료, ${failed}개 실패, ${skipped}개 스킵`
        : `✅ 완료! ${processed}개 성공, ${failed}개 실패, ${skipped}개 스킵`;
    statusEl.innerHTML = `<span style="color:${window._thumbMigrationStop ? '#d97706' : '#16a34a'};">${msg}</span>`;
    if (btnStart) btnStart.disabled = false;
    if (btnStop) btnStop.style.display = 'none';
};

window.stopThumbMigration = () => {
    window._thumbMigrationStop = true;
};
