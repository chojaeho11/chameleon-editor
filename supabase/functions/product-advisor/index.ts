...
- 고객이 "원판", "원장", "허니콤보드 원판", "허니콤보드 원장", "raw board", "원자재", "가공 전 보드" 등 가공 전 허니콤보드 원판을 구매하고 싶어 하면:
  ...
  - ★ **원판 문의 연락처**: 본사 031-366-1984, 매니저 010-3455-1946/010-7793-5393/010-4994-5950/010-2525-9943, 이메일 design@chameleon.design
  ...
  - 담당 매니저: ${mgrName || '배정 중'}${mgrPhone ? (' (' + mgrPhone + ')') : ''}`;
                ...
                if (orderId && !_msgPhone) {
                    ...
                    if (!list.length) {
                        ...
                        orderContext = `[주문조회시스템 — 실제 DB 조회 결과다. 이 정보만 사용하고, 없는 항목은 지어내지 마라.]\n휴대폰 ${providedFullPhone} 로 접수된 최근 주문 ${list.length > 5 ? '5건(이상)' : list.length + '건'}:\n${lines.join('\n')}\n  - 본사: 031-366-1984\n\n⚠️ 출력 지침 (매우 중요): 위 주문 목록을 채팅 답변에 그대로 보기 좋게 나열해서 고객에게 직접 보여줘라. 각 주문마다 주문번호·상품·진행 상태·배송 예정일·결제 상태를 실제로 적을 것 (최신순). "정리해 드렸습니다 / 확인해 주세요" 처럼 내용을 생략한 멘트만 하는 것은 절대 금지 — 반드시 실제 목록을 적어라. 추가 문의는 담당 매니저(번호 있으면 포함) 또는 본사로 안내. '입금 전'이면 입금 계좌 안내 포함. 고객의 언어로 답해라.`;
                    }
                } else {
