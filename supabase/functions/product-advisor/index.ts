...
8. **허니콤보드 견적 흐름 (매우 중요!)**:
   ...
   - ★ **가벽은 95%가 단면!** 단면/양면을 묻지 말고 **단면 기준으로 바로 견적**을 내줘!
     · 견적 후 안내: "혹시 양면, 코너기둥, 보조받침대 등 다른 옵션이 필요하시면 말씀해 주세요."
     · **코너기둥**: ㄱ자, ㄷ자 형태로 가벽을 연결해서 넓은 공간을 만들 때 필요
     · **보조받침대**: 아이들이 많거나 야외 바람 부는 곳에서는 필수
   - ★ **배너도 95%가 단면!** 양면은 별도로 안내하지 마. 고객이 "양면 되냐"고 물을 때만 안내.
     · 사이즈와 수량만 확인하면 바로 견적!
   - 인쇄커팅: 사각/모양커팅만 물어봐 (단면 기준)
   - ★ 사이즈 + 수량이 나오면 **바로 견적서 생성**! 불필요한 질문 최소화!
        ...
        async function callClaude(model: string, retries = 0): Promise<any> {
            ...
            if (toolBlock && toolBlock.name === "generate_quote") {
                ...
                for (const qi of qItems) {
                    ...
                    // ★ 허니콤보드: 커팅 옵션 자동 추가 (배너 제외! 배너는 커팅 없음)
                    const _isBanner = (qi.code || '').startsWith('hb_bn');
                    if (_isHoneycomb && !_isBanner) {
