        ...
        try {
            ...
            sb.auth.onAuthStateChange((event, session) => {
                ...
                if (event === 'SIGNED_IN' && session && session.user) {
                    ...
                    // ★ 소셜 로그인 후 결제 재개: 결제 모달 바로 복원
                    if (sessionStorage.getItem('_pendingPayment')) {
                        setTimeout(() => {
...
async function loadSystemData() {
    try {
        ...
        const _cacheKey = 'chameleon_addons_cache_v5';
        ...
        try { localStorage.removeItem('chameleon_addons_cache_v3'); } catch(e) {} // v3 구버전 캐시 제거 (2026-06-17 선반 30K 가격 변경)
        try {
