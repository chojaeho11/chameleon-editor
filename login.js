...
import { sb, currentUser, isAdmin } from "./config.js?v=444";
...
export function initAuth() {
    ...
    // KR: 카카오 표시 (기본), JP/해외: Apple 표시. LINE 은 콜백 미등록 이슈로 숨김(사용자 요청 2026-07-15).
    if (country === 'JP') {
    ...
    try {
        if (isSignUpMode) {
            ...
            if (data.user) {
                ...
                const SIGNUP_BONUS_KRW = 30000;  // 2026-07-15: 1만→3만 재통일 (사장님 요청 — 마케팅 표기(3만원)와 실제 지급 일치). JP=3,000엔/US=$30 통화 자동 환산.
