...
// ─── 견적 마법사 (Quote Wizard) ───
window.openQuoteWizard = async function() {
    if (!chatArea) { chatArea = document.querySelector('.adv-chat-area') || document.getElementById('advChatArea'); }
    if (!chatArea) { console.error('chatArea not found'); return; }

    // Remove existing wizard if any
    const existing = chatArea.querySelector('.qw-wizard-card');
    ...
    // ── i18n for wizard ──
    const QW = {
        kr: { title:'견적 마법사', close:'닫기', width:'가로', height:'높이', print:'인쇄', single:'단면', double:'양면', linked:'연결형', qty:'수량', addOpt:'추가옵션', shipping:'배송', seoulFree:'서울/경기 무료', localFee:'지방', localHb:'+200,000원', localBn:'+30,000원', total:'합계', generate:'견적서 생성', loading:'데이터 로딩중...', cutting:'커팅', shapeCut:'모양커팅 +3,000원', rectCut:'사각커팅 +1,000원', fabric:'원단', finish:'마감', loop:'고리', perSqm:'/㎡', type:'타입', support:'보조받침대', light:'조명', corner:'코너기둥', tab1:'허니콤 가벽', tab2:'허니콤 배너', tab3:'등신대', tab4:'패브릭 인쇄', discount:'할인', subtotal:'소계', shippingFee:'배송비', volumeDisc:'대량할인', mm:'mm', ea:'개', won:'원' },
        ja: { title:'見積ウィザード', close:'閉じる', width:'幅', height:'高さ', print:'印刷', single:'片面', double:'両面', linked:'連結型', qty:'数量', addOpt:'追加オプション', shipping:'配送', seoulFree:'首都圏 無料', localFee:'地方', localHb:'+200,000円', localBn:'+30,000円', total:'合計', generate:'見積書作成', loading:'データ読み込み中...', cutting:'カット', shapeCut:'型抜き +3,000円', rectCut:'四角カット +1,000円', fabric:'生地', finish:'仕上げ', loop:'ループ', perSqm:'/㎡', type:'タイプ', support:'補助スタンド', light:'照明', corner:'コーナーポール', tab1:'ハニカム間仕切り', tab2:'ハニカムバナー', tab3:'等身大パネル', tab4:'ファブリック印刷', discount:'割引', subtotal:'小計', shippingFee:'送料', volumeDisc:'ボリューム割引', mm:'mm', ea:'個', won:'円' },
        en: { title:'Quote Wizard', close:'Close', width:'Width', height:'Height', print:'Print', single:'Single-side', double:'Double-side', linked:'Linked', qty:'Quantity', addOpt:'Add-ons', shipping:'Shipping', seoulFree:'Seoul/Gyeonggi Free', localFee:'Regional', localHb:'+200,000 KRW', localBn:'+30,000 KRW', total:'Total', generate:'Generate Quote', loading:'Loading data...', cutting:'Cutting', shapeCut:'Shape cut +3,000', rectCut:'Rectangle +1,000', fabric:'Fabric', finish:'Finishing', loop:'Loops', perSqm:'/sqm', type:'Type', support:'Support Stand', light:'Lighting', corner:'Corner Pole', tab1:'Honeycomb Wall', tab2:'Honeycomb Banner', tab3:'Life-size Panel', tab4:'Fabric Print', discount:'Discount', subtotal:'Subtotal', shippingFee:'Shipping', volumeDisc:'Volume Discount', mm:'mm', ea:'pcs', won:'KRW' },
    };
    function qw(k) { return (QW[lang] && QW[lang][k]) || QW['en'][k] || k; }

    // ── Fetch DB data ──
    let hbProducts = [], hbAddons = [], fabricProducts = [], fabricAddons = [];
    try {
        const [r1, r2, r3, r4] = await Promise.all([
            sb.from('admin_products').select('code, name, price, is_custom_size, category').in('code', ['hb_dw_1','hb_bn_1','hb_bn_2','hb_bn_3','hb_pi_5']),
            sb.from('admin_addons').select('code, name, price_kr').in('code', ['b0001','87545','For']),
            sb.from('admin_products').select('code, name, price, is_custom_size, category').in('category', ['ch10s','ch20s','ch40s','cn16s','obo10s','lin20s']),
            sb.from('admin_addons').select('code, name, price_kr').in('code', ['txl0001','txl0002','txl0003','txl0004','txl0005','45783','45722','45787','3254352','45646456','3453453','355353']),
        ]);
        hbProducts = r1.data || [];
        hbAddons = r2.data || [];
        fabricProducts = r3.data || [];
        fabricAddons = r4.data || [];
    } catch(e) { console.error('Quote wizard DB fetch error:', e); }

    // Helper: find product/addon by code
    function findProd(code) { return hbProducts.find(p => p.code === code) || fabricProducts.find(p => p.code === code); }
    function findAddon(code) { return hbAddons.find(a => a.code === code) || fabricAddons.find(a => a.code === code); }
    function addonPrice(code) { const a = findAddon(code); return a ? (a.price_kr || 0) : 0; }
    function prodPrice(code) { const p = findProd(code); return p ? (p.price || 0) : 0; }

    // Fabric category → display name mapping
    const fabricMap = [
        { cat: 'ch10s', name: lang === 'ja' ? '広木10手' : lang === 'en' ? 'Cotton 10s' : '광목10수' },
        { cat: 'ch20s', name: lang === 'ja' ? '広木20手ホワイト' : lang === 'en' ? 'Cotton 20s White' : '광목20수화이트' },
        { cat: 'ch40s', name: lang === 'ja' ? '広木40手' : lang === 'en' ? 'Cotton 40s' : '광목40수' },
        { cat: 'cn16s', name: lang === 'ja' ? 'キャンバス16手' : lang === 'en' ? 'Canvas 16s' : '캔버스16수' },
        { cat: 'obo10s', name: lang === 'ja' ? 'オックスフォード10手' : lang === 'en' ? 'Oxford 10s' : '옥스포드10수' },
        { cat: 'lin20s', name: lang === 'ja' ? 'リネン' : lang === 'en' ? 'Linen' : '리넨' },
    ];
    function fabricPriceByCat(cat) { const p = fabricProducts.find(fp => fp.category === cat); return p ? p.price : 0; }
    function fabricCodeByCat(cat) { const p = fabricProducts.find(fp => fp.category === cat); return p ? p.code : cat; }

    // Fabric finishing addons
    const finishAddons = [
        { code: 'txl0001', name: lang === 'ja' ? '裁断' : lang === 'en' ? 'Raw cut' : '가재단' },
        { code: 'txl0002', name: lang === 'ja' ? 'オーバーロック' : lang === 'en' ? 'Overlock' : '오버록' },
        { code: 'txl0003', name: lang === 'ja' ? 'インターロック' : lang === 'en' ? 'Interlock' : '인터록' },
        { code: 'txl0004', name: lang === 'ja' ? '巻き縫い' : lang === 'en' ? 'Roll stitch' : '말아박기' },
    ];
    const loopAddons = [
        { code: 'txl0005', name: lang === 'ja' ? '上部紐ループ' : lang === 'en' ? 'Top string loop' : '상단끈고리' },
        { code: '45783', name: lang === 'ja' ? 'サスペンダーループ' : lang === 'en' ? 'Suspender loop' : '멜빵고리' },
        { code: '45722', name: lang === 'ja' ? '上部棒仕上げ' : lang === 'en' ? 'Top rod finish' : '상단봉마감' },
        { code: '45787', name: lang === 'ja' ? '棒カーテン仕上げ' : lang === 'en' ? 'Rod curtain finish' : '봉커튼마감' },
    ];

    // ── State ──
    const state = {
        tab: 0,
        // Tab 0: honeycomb wall
        hb_w: 3, hb_h: 2.4, hb_side: 1, hb_qty: 1,
        hb_addon_support: false, hb_addon_support_qty: 1,
        hb_addon_light: false, hb_addon_light_qty: 1,
        hb_addon_corner: false, hb_addon_corner_qty: 1,
        hb_shipping: 0,
        // Tab 1: honeycomb banner
        bn_w: 1000, bn_h: 2000, bn_type: 'single', bn_qty: 1, bn_shipping: 0,
        // Tab 2: life-size panel
        pi_w: 600, pi_h: 1800, pi_cut: 'shape', pi_qty: 1, pi_shipping: 0,
        // Tab 3: fabric
        fb_cat: 'ch10s', fb_w: 1000, fb_h: 1000, fb_qty: 1,
        fb_finish: {}, fb_loop: {},
    };

    // ── Price calculation ──
    function calcPrice() {
        let result = { items: [], subtotal: 0, discount: 0, shipping: 0, total: 0 };
        if (state.tab === 0) {
            // Honeycomb wall: width in meters, height in meters
            const w_mm = state.hb_w * 1000;
            const h_mm = state.hb_h * 1000;
            const area = (w_mm * h_mm) / 1000000;
            const dbPrice = prodPrice('hb_dw_1');
            let unit = Math.floor(area * dbPrice * state.hb_side / 100) * 100;
            unit = Math.max(unit, 10000);
            const lineTotal = unit * state.hb_qty;
            result.items.push({ name: qw('tab1'), spec: state.hb_w + 'm x ' + state.hb_h + 'm ' + (state.hb_side === 2 ? qw('double') : qw('single')), qty: state.hb_qty, unit_price: unit, total: lineTotal, _code: 'hb_dw_1', _width_mm: w_mm, _height_mm: h_mm, is_addon: false });
            result.subtotal += lineTotal;
            // Addons
            if (state.hb_addon_support) {
                const ap = addonPrice('b0001');
                const at = ap * state.hb_addon_support_qty;
                result.items.push({ name: qw('support'), spec: '', qty: state.hb_addon_support_qty, unit_price: ap, total: at, _code: 'b0001', is_addon: true });
                result.subtotal += at;
            }
            if (state.hb_addon_light) {
                const ap = addonPrice('87545');
                const at = ap * state.hb_addon_light_qty;
                result.items.push({ name: qw('light'), spec: '', qty: state.hb_addon_light_qty, unit_price: ap, total: at, _code: '87545', is_addon: true });
                result.subtotal += at;
            }
            if (state.hb_addon_corner) {
                const ap = addonPrice('For');
                const at = ap * state.hb_addon_corner_qty;
                result.items.push({ name: qw('corner'), spec: '', qty: state.hb_addon_corner_qty, unit_price: ap, total: at, _code: 'For', is_addon: true });
                result.subtotal += at;
            }
            // Volume discount on honeycomb main item total (before addons)
            const hbMain = lineTotal;
            let discPct = 0;
            if (hbMain >= 10000000) discPct = 30;
            else if (hbMain >= 7000000) discPct = 25;
            else if (hbMain >= 5000000) discPct = 20;
            else if (hbMain >= 3000000) discPct = 15;
            else if (hbMain >= 2000000) discPct = 10;
            if (discPct > 0) {
                const discAmt = Math.floor(hbMain * discPct / 100 / 100) * 100;
                result.discount = discAmt;
                result.items.push({ name: qw('volumeDisc') + ' ' + discPct + '%', spec: '', qty: 1, unit_price: -discAmt, total: -discAmt, is_addon: true });
            }
            result.shipping = state.hb_shipping;
        } else if (state.tab === 1) {
            // Honeycomb banner
            const bnCode = state.bn_type === 'double' ? 'hb_bn_2' : state.bn_type === 'linked' ? 'hb_bn_3' : 'hb_bn_1';
            const area = (state.bn_w * state.bn_h) / 1000000;
            const dbPrice = prodPrice(bnCode);
            let unit = Math.floor(area * dbPrice / 100) * 100;
            unit = Math.max(unit, 10000);
            const lineTotal = unit * state.bn_qty;
            result.items.push({ name: qw('tab2'), spec: state.bn_w + 'mm x ' + state.bn_h + 'mm ' + (state.bn_type === 'double' ? qw('double') : state.bn_type === 'linked' ? qw('linked') : qw('single')), qty: state.bn_qty, unit_price: unit, total: lineTotal, _code: bnCode, _width_mm: state.bn_w, _height_mm: state.bn_h, is_addon: false });
            result.subtotal += lineTotal;
            result.shipping = state.bn_shipping;
        } else if (state.tab === 2) {
            // Life-size panel
            const area = (state.pi_w * state.pi_h) / 1000000;
            const dbPrice = prodPrice('hb_pi_5');
            let unit = Math.floor(area * dbPrice / 100) * 100;
            unit = Math.max(unit, 10000);
            const cutFee = state.pi_cut === 'shape' ? 3000 : 1000;
            const unitWithCut = unit + cutFee;
            const lineTotal = unitWithCut * state.pi_qty;
            result.items.push({ name: qw('tab3'), spec: state.pi_w + 'mm x ' + state.pi_h + 'mm ' + (state.pi_cut === 'shape' ? (lang === 'kr' ? '모양커팅' : lang === 'ja' ? '型抜き' : 'Shape cut') : (lang === 'kr' ? '사각커팅' : lang === 'ja' ? '四角カット' : 'Rectangle')), qty: state.pi_qty, unit_price: unitWithCut, total: lineTotal, _code: 'hb_pi_5', _width_mm: state.pi_w, _height_mm: state.pi_h, is_addon: false });
            result.subtotal += lineTotal;
            result.shipping = state.pi_shipping;
        } else if (state.tab === 3) {
            // Fabric
            const area = (state.fb_w * state.fb_h) / 1000000;
            const dbPrice = fabricPriceByCat(state.fb_cat);
            let unit = Math.floor(area * dbPrice / 100) * 100;
            unit = Math.max(unit, 10000);
            const lineTotal = unit * state.fb_qty;
            const fName = (fabricMap.find(f => f.cat === state.fb_cat) || {}).name || state.fb_cat;
            result.items.push({ name: qw('tab4') + ' (' + fName + ')', spec: state.fb_w + 'mm x ' + state.fb_h + 'mm', qty: state.fb_qty, unit_price: unit, total: lineTotal, _code: fabricCodeByCat(state.fb_cat), _width_mm: state.fb_w, _height_mm: state.fb_h, is_addon: false });
            result.subtotal += lineTotal;
            // Finishing addons (per-meter pricing based on perimeter or width)
            for (const fa of finishAddons) {
                if (state.fb_finish[fa.code]) {
                    const ap = addonPrice(fa.code);
                    const at = ap * state.fb_qty;
                    result.items.push({ name: fa.name, spec: '', qty: state.fb_qty, unit_price: ap, total: at, _code: fa.code, is_addon: true });
                    result.subtotal += at;
                }
            }
            for (const la of loopAddons) {
                if (state.fb_loop[la.code]) {
                    const ap = addonPrice(la.code);
                    const at = ap * state.fb_qty;
                    result.items.push({ name: la.name, spec: '', qty: state.fb_qty, unit_price: ap, total: at, _code: la.code, is_addon: true });
                    result.subtotal += at;
                }
            }
        }
    ...
    card.className = 'adv-row adv-row-ai qw-wizard-card';
    card.style.cssText = 'width:100%;';

    function render() {
        const price = calcPrice();
        const tabs = [
            { icon: '\u{1F3D7}\uFE0F', label: qw('tab1') },
            { icon: '\u{1F3F3}\uFE0F', label: qw('tab2') },
            { icon: '\u{1F9CD}', label: qw('tab3') },
            { icon: '\u{1F9F5}', label: qw('tab4') },
        ];
        let tabsHtml = '<div style="display:flex;gap:4px;overflow-x:auto;margin-bottom:14px;padding-bottom:4px;">';
        tabs.forEach((tb, i) => {
            const active = state.tab === i;
            tabsHtml += `<button class="qw-tab" data-tab="${i}" style="flex:none;padding:8px 14px;border-radius:20px;border:1.5px solid ${active ? '#6366f1' : '#444'};background:${active ? '#6366f1' : 'transparent'};color:${active ? '#fff' : '#ccc'};font-size:12px;font-weight:${active ? '700' : '500'};cursor:pointer;white-space:nowrap;font-family:inherit;">${tb.icon} ${tb.label}</button>`;
        });
        tabsHtml += '</div>';
        ...
        if (state.tab === 0) {
            ...
            const heights = [2, 2.2, 2.4, 3];
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('width')}</div><div style="display:flex;flex-wrap:wrap;gap:6px;">`;
            widths.forEach(w => {
                const sel = state.hb_w === w;
                bodyHtml += `<button class="qw-size-btn" data-key="hb_w" data-val="${w}" style="padding:8px 14px;border-radius:8px;border:1.5px solid ${sel ? '#6366f1' : '#555'};background:${sel ? '#6366f1' : 'transparent'};color:${sel ? '#fff' : '#ccc'};font-size:13px;cursor:pointer;font-family:inherit;">${w}m</button>`;
            });
            bodyHtml += '</div></div>';

            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('height')}</div><div style="display:flex;flex-wrap:wrap;gap:6px;">`;
            heights.forEach(h => {
                const sel = state.hb_h === h;
                bodyHtml += `<button class="qw-size-btn" data-key="hb_h" data-val="${h}" style="padding:8px 14px;border-radius:8px;border:1.5px solid ${sel ? '#6366f1' : '#555'};background:${sel ? '#6366f1' : 'transparent'};color:${sel ? '#fff' : '#ccc'};font-size:13px;cursor:pointer;font-family:inherit;">${h}m</button>`;
            });
            bodyHtml += '</div></div>';

            // Print side
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('print')}</div><div style="display:flex;gap:8px;">`;
            [1,2].forEach(s => {
                const sel = state.hb_side === s;
                bodyHtml += `<button class="qw-side-btn" data-val="${s}" style="flex:1;padding:8px;border-radius:8px;border:1.5px solid ${sel ? '#6366f1' : '#555'};background:${sel ? '#6366f1' : 'transparent'};color:${sel ? '#fff' : '#ccc'};font-size:13px;cursor:pointer;font-family:inherit;">${s === 1 ? qw('single') : qw('double')}</button>`;
            });
            bodyHtml += '</div></div>';

            // Quantity
            bodyHtml += _qtyHtml('hb_qty', state.hb_qty);

            // Addons
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('addOpt')}</div>`;
            const addonDefs = [
                { key: 'hb_addon_support', qtyKey: 'hb_addon_support_qty', code: 'b0001', label: qw('support') },
                { key: 'hb_addon_light', qtyKey: 'hb_addon_light_qty', code: '87545', label: qw('light') },
                { key: 'hb_addon_corner', qtyKey: 'hb_addon_corner_qty', code: 'For', label: qw('corner') },
            ];
            addonDefs.forEach(ad => {
                const checked = state[ad.key];
                const ap = addonPrice(ad.code);
                bodyHtml += `<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                    <label style="display:flex;align-items:center;gap:6px;cursor:pointer;color:#ccc;font-size:13px;">
                        <input type="checkbox" class="qw-addon-chk" data-key="${ad.key}" ${checked ? 'checked' : ''} style="accent-color:#6366f1;">
                        ${ad.label} <span style="color:#94a3b8;font-size:11px;">(${ap.toLocaleString()}${qw('won')})</span>
                    </label>
                    ${checked ? `<div style="display:flex;align-items:center;gap:4px;"><button class="qw-addon-minus" data-key="${ad.qtyKey}" style="width:24px;height:24px;border-radius:6px;border:1px solid #555;background:transparent;color:#ccc;cursor:pointer;font-size:14px;">-</button><span style="color:#fff;font-size:13px;min-width:20px;text-align:center;">${state[ad.qtyKey]}</span><button class="qw-addon-plus" data-key="${ad.qtyKey}" style="width:24px;height:24px;border-radius:6px;border:1px solid #555;background:transparent;color:#ccc;cursor:pointer;font-size:14px;">+</button></div>` : ''}
                </div>`;
            });
            bodyHtml += '</div>';

            // Shipping
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('shipping')}</div><div style="display:flex;gap:8px;">`;
            bodyHtml += `<button class="qw-ship-btn" data-val="0" style="flex:1;padding:8px;border-radius:8px;border:1.5px solid ${state.hb_shipping === 0 ? '#6366f1' : '#555'};background:${state.hb_shipping === 0 ? '#6366f1' : 'transparent'};color:${state.hb_shipping === 0 ? '#fff' : '#ccc'};font-size:12px;cursor:pointer;font-family:inherit;">${qw('seoulFree')}</button>`;
            bodyHtml += `<button class="qw-ship-btn" data-val="200000" style="flex:1;padding:8px;border-radius:8px;border:1.5px solid ${state.hb_shipping === 200000 ? '#6366f1' : '#555'};background:${state.hb_shipping === 200000 ? '#6366f1' : 'transparent'};color:${state.hb_shipping === 200000 ? '#fff' : '#ccc'};font-size:12px;cursor:pointer;font-family:inherit;">${qw('localFee')} ${qw('localHb')}</button>`;
            bodyHtml += '</div></div>';
        } else if (state.tab === 1) {
            // Honeycomb banner
            bodyHtml += _numInputHtml('bn_w', qw('width'), state.bn_w, 'mm');
            bodyHtml += _numInputHtml('bn_h', qw('height'), state.bn_h, 'mm');
            // Type
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('type')}</div><div style="display:flex;gap:6px;">`;
            ['single','double','linked'].forEach(tp => {
                const sel = state.bn_type === tp;
                const lbl = tp === 'single' ? qw('single') : tp === 'double' ? qw('double') : qw('linked');
                bodyHtml += `<button class="qw-bntype-btn" data-val="${tp}" style="flex:1;padding:8px;border-radius:8px;border:1.5px solid ${sel ? '#6366f1' : '#555'};background:${sel ? '#6366f1' : 'transparent'};color:${sel ? '#fff' : '#ccc'};font-size:12px;cursor:pointer;font-family:inherit;">${lbl}</button>`;
            });
            bodyHtml += '</div></div>';
            bodyHtml += _qtyHtml('bn_qty', state.bn_qty);
            // Shipping
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('shipping')}</div><div style="display:flex;gap:8px;">`;
            bodyHtml += `<button class="qw-bnship-btn" data-val="0" style="flex:1;padding:8px;border-radius:8px;border:1.5px solid ${state.bn_shipping === 0 ? '#6366f1' : '#555'};background:${state.bn_shipping === 0 ? '#6366f1' : 'transparent'};color:${state.bn_shipping === 0 ? '#fff' : '#ccc'};font-size:12px;cursor:pointer;font-family:inherit;">${qw('seoulFree')}</button>`;
            bodyHtml += `<button class="qw-bnship-btn" data-val="30000" style="flex:1;padding:8px;border-radius:8px;border:1.5px solid ${state.bn_shipping === 30000 ? '#6366f1' : '#555'};background:${state.bn_shipping === 30000 ? '#6366f1' : 'transparent'};color:${state.bn_shipping === 30000 ? '#fff' : '#ccc'};font-size:12px;cursor:pointer;font-family:inherit;">${qw('localFee')} ${qw('localBn')}</button>`;
            bodyHtml += '</div></div>';
        } else if (state.tab === 2) {
            // Life-size panel
            bodyHtml += _numInputHtml('pi_w', qw('width'), state.pi_w, 'mm');
            bodyHtml += _numInputHtml('pi_h', qw('height'), state.pi_h, 'mm');
            // Cutting
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('cutting')}</div><div style="display:flex;gap:8px;">`;
            ['shape','rect'].forEach(ct => {
                const sel = state.pi_cut === ct;
                const lbl = ct === 'shape' ? qw('shapeCut') : qw('rectCut');
                bodyHtml += `<button class="qw-picut-btn" data-val="${ct}" style="flex:1;padding:8px;border-radius:8px;border:1.5px solid ${sel ? '#6366f1' : '#555'};background:${sel ? '#6366f1' : 'transparent'};color:${sel ? '#fff' : '#ccc'};font-size:12px;cursor:pointer;font-family:inherit;">${lbl}</button>`;
            });
            bodyHtml += '</div></div>';
            bodyHtml += _qtyHtml('pi_qty', state.pi_qty);
            // Shipping
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('shipping')}</div><div style="display:flex;gap:8px;">`;
            bodyHtml += `<button class="qw-piship-btn" data-val="0" style="flex:1;padding:8px;border-radius:8px;border:1.5px solid ${state.pi_shipping === 0 ? '#6366f1' : '#555'};background:${state.pi_shipping === 0 ? '#6366f1' : 'transparent'};color:${state.pi_shipping === 0 ? '#fff' : '#ccc'};font-size:12px;cursor:pointer;font-family:inherit;">${qw('seoulFree')}</button>`;
            bodyHtml += `<button class="qw-piship-btn" data-val="30000" style="flex:1;padding:8px;border-radius:8px;border:1.5px solid ${state.pi_shipping === 30000 ? '#6366f1' : '#555'};background:${state.pi_shipping === 30000 ? '#6366f1' : 'transparent'};color:${state.pi_shipping === 30000 ? '#fff' : '#ccc'};font-size:12px;cursor:pointer;font-family:inherit;">${qw('localFee')} ${qw('localBn')}</button>`;
            bodyHtml += '</div></div>';
        } else if (state.tab === 3) {
            // Fabric
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('fabric')}</div>`;
            bodyHtml += `<select class="qw-fabric-sel" style="width:100%;padding:10px;border-radius:8px;border:1.5px solid #555;background:#2a2a3e;color:#fff;font-size:13px;font-family:inherit;">`;
            fabricMap.forEach(f => {
                const fp = fabricPriceByCat(f.cat);
                bodyHtml += `<option value="${f.cat}" ${state.fb_cat === f.cat ? 'selected' : ''}>${f.name} (${fp.toLocaleString()}${qw('won')}${qw('perSqm')})</option>`;
            });
            bodyHtml += '</select></div>';

            bodyHtml += _numInputHtml('fb_w', qw('width'), state.fb_w, 'mm');
            bodyHtml += _numInputHtml('fb_h', qw('height'), state.fb_h, 'mm');
            bodyHtml += _qtyHtml('fb_qty', state.fb_qty);

            // Finishing
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('finish')}</div>`;
            finishAddons.forEach(fa => {
                const checked = !!state.fb_finish[fa.code];
                const ap = addonPrice(fa.code);
                bodyHtml += `<label style="display:flex;align-items:center;gap:6px;margin-bottom:4px;cursor:pointer;color:#ccc;font-size:13px;">
                    <input type="checkbox" class="qw-fb-finish" data-code="${fa.code}" ${checked ? 'checked' : ''} style="accent-color:#6366f1;">
                    ${fa.name} <span style="color:#94a3b8;font-size:11px;">(${ap.toLocaleString()}${qw('won')})</span>
                </label>`;
            });
            bodyHtml += '</div>';

            // Loops
            bodyHtml += `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('loop')}</div>`;
            loopAddons.forEach(la => {
                const checked = !!state.fb_loop[la.code];
                const ap = addonPrice(la.code);
                bodyHtml += `<label style="display:flex;align-items:center;gap:6px;margin-bottom:4px;cursor:pointer;color:#ccc;font-size:13px;">
                    <input type="checkbox" class="qw-fb-loop" data-code="${la.code}" ${checked ? 'checked' : ''} style="accent-color:#6366f1;">
                    ${la.name} <span style="color:#94a3b8;font-size:11px;">(${ap.toLocaleString()}${qw('won')})</span>
                </label>`;
            });
            bodyHtml += '</div>';
        }
        ...
        // Price summary
        let summaryHtml = '<div style="background:#1a1a2e;border-radius:10px;padding:12px;margin-top:8px;">';
        price.items.forEach(it => {
            summaryHtml += `<div style="display:flex;justify-content:space-between;font-size:12px;color:${it.total < 0 ? '#f87171' : '#ccc'};margin-bottom:3px;"><span>${it.name} ${it.spec ? '(' + it.spec + ')' : ''} x${it.qty}</span><span>${it.total.toLocaleString()}${qw('won')}</span></div>`;
        });
        if (price.shipping > 0) {
            summaryHtml += `<div style="display:flex;justify-content:space-between;font-size:12px;color:#fbbf24;margin-bottom:3px;"><span>${qw('shippingFee')}</span><span>+${price.shipping.toLocaleString()}${qw('won')}</span></div>`;
        }
        summaryHtml += `<div style="border-top:1px solid #444;margin-top:6px;padding-top:6px;display:flex;justify-content:space-between;font-size:16px;font-weight:800;color:#fff;"><span>${qw('total')}</span><span>${price.total.toLocaleString()}${qw('won')}</span></div>`;
        summaryHtml += '</div>';

        // Buttons
        const btnHtml = `<div style="display:flex;gap:8px;margin-top:12px;">
            <button class="qw-close-btn" style="flex:1;padding:12px;border-radius:10px;border:1.5px solid #555;background:transparent;color:#ccc;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">${qw('close')}</button>
            <button class="qw-generate-btn" style="flex:2;padding:12px;border-radius:10px;border:none;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;">${qw('generate')}</button>
        </div>`;

        card.innerHTML = `
            <div class="adv-avatar"><i class="fa-solid fa-calculator"></i></div>
            <div style="background:#1e1e2e;border:1px solid #333;border-radius:14px;padding:16px;max-width:92%;width:100%;box-sizing:border-box;">
                <div style="font-size:15px;font-weight:800;color:#fff;margin-bottom:12px;"><i class="fa-solid fa-wand-magic-sparkles" style="color:#a5b4fc;"></i> ${qw('title')}</div>
                ${tabsHtml}
                ${bodyHtml}
                ${summaryHtml}
                ${btnHtml}
    ...
    function _qtyHtml(key, val) {
        return `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${qw('qty')}</div>
        <div style="display:flex;align-items:center;gap:8px;">
            <button class="qw-qty-minus" data-key="${key}" style="width:32px;height:32px;border-radius:8px;border:1.5px solid #555;background:transparent;color:#ccc;font-size:18px;cursor:pointer;">-</button>
            <input type="number" class="qw-qty-input" data-key="${key}" value="${val}" min="1" style="width:60px;text-align:center;padding:6px;border-radius:8px;border:1.5px solid #555;background:#2a2a3e;color:#fff;font-size:14px;font-family:inherit;">
            <button class="qw-qty-plus" data-key="${key}" style="width:32px;height:32px;border-radius:8px;border:1.5px solid #555;background:transparent;color:#ccc;font-size:18px;cursor:pointer;">+</button>
        </div></div>`;
    }

    function _numInputHtml(key, label, val, unit) {
        return `<div style="margin-bottom:12px;"><div style="font-size:12px;font-weight:700;color:#a5b4fc;margin-bottom:6px;">${label} (${unit})</div>
        <input type="number" class="qw-num-input" data-key="${key}" value="${val}" min="1" style="width:100%;padding:10px;border-radius:8px;border:1.5px solid #555;background:#2a2a3e;color:#fff;font-size:14px;font-family:inherit;box-sizing:border-box;"></div>`;
    }

    function bindEvents() {
        // Tabs
        card.querySelectorAll('.qw-tab').forEach(btn => {
            btn.addEventListener('click', () => { state.tab = parseInt(btn.dataset.tab); render(); });
        });
        // Size buttons (hb_w, hb_h)
        card.querySelectorAll('.qw-size-btn').forEach(btn => {
            btn.addEventListener('click', () => { state[btn.dataset.key] = parseFloat(btn.dataset.val); render(); });
        });
        // Side buttons
        card.querySelectorAll('.qw-side-btn').forEach(btn => {
            btn.addEventListener('click', () => { state.hb_side = parseInt(btn.dataset.val); render(); });
        });
        // Qty +/- and input
        card.querySelectorAll('.qw-qty-minus').forEach(btn => {
            btn.addEventListener('click', () => { state[btn.dataset.key] = Math.max(1, (state[btn.dataset.key] || 1) - 1); render(); });
        });
        card.querySelectorAll('.qw-qty-plus').forEach(btn => {
            btn.addEventListener('click', () => { state[btn.dataset.key] = (state[btn.dataset.key] || 1) + 1; render(); });
        });
        card.querySelectorAll('.qw-qty-input').forEach(inp => {
            inp.addEventListener('change', () => { state[inp.dataset.key] = Math.max(1, parseInt(inp.value) || 1); render(); });
        });
        // Addon checkboxes
        card.querySelectorAll('.qw-addon-chk').forEach(chk => {
            chk.addEventListener('change', () => { state[chk.dataset.key] = chk.checked; render(); });
        });
        // Addon qty +/-
        card.querySelectorAll('.qw-addon-minus').forEach(btn => {
            btn.addEventListener('click', () => { state[btn.dataset.key] = Math.max(1, (state[btn.dataset.key] || 1) - 1); render(); });
        });
        card.querySelectorAll('.qw-addon-plus').forEach(btn => {
            btn.addEventListener('click', () => { state[btn.dataset.key] = (state[btn.dataset.key] || 1) + 1; render(); });
        });
        // Shipping buttons (hb)
        card.querySelectorAll('.qw-ship-btn').forEach(btn => {
            btn.addEventListener('click', () => { state.hb_shipping = parseInt(btn.dataset.val); render(); });
        });
        // Banner type
        card.querySelectorAll('.qw-bntype-btn').forEach(btn => {
            btn.addEventListener('click', () => { state.bn_type = btn.dataset.val; render(); });
        });
        // Banner shipping
        card.querySelectorAll('.qw-bnship-btn').forEach(btn => {
            btn.addEventListener('click', () => { state.bn_shipping = parseInt(btn.dataset.val); render(); });
        });
        // Panel cutting
        card.querySelectorAll('.qw-picut-btn').forEach(btn => {
            btn.addEventListener('click', () => { state.pi_cut = btn.dataset.val; render(); });
        });
        // Panel shipping
        card.querySelectorAll('.qw-piship-btn').forEach(btn => {
            btn.addEventListener('click', () => { state.pi_shipping = parseInt(btn.dataset.val); render(); });
        });
        // Number inputs (bn_w, bn_h, pi_w, pi_h, fb_w, fb_h)
        card.querySelectorAll('.qw-num-input').forEach(inp => {
            inp.addEventListener('change', () => { state[inp.dataset.key] = Math.max(1, parseInt(inp.value) || 1); render(); });
        });
        // Fabric select
        const fbSel = card.querySelector('.qw-fabric-sel');
        if (fbSel) fbSel.addEventListener('change', () => { state.fb_cat = fbSel.value; render(); });
        // Fabric finishing
        card.querySelectorAll('.qw-fb-finish').forEach(chk => {
            chk.addEventListener('change', () => { if (chk.checked) state.fb_finish[chk.dataset.code] = true; else delete state.fb_finish[chk.dataset.code]; render(); });
        });
        // Fabric loops
        card.querySelectorAll('.qw-fb-loop').forEach(chk => {
            chk.addEventListener('change', () => { if (chk.checked) state.fb_loop[chk.dataset.code] = true; else delete state.fb_loop[chk.dataset.code]; render(); });
        });
        // Close
        const closeBtn = card.querySelector('.qw-close-btn');
        if (closeBtn) closeBtn.addEventListener('click', () => { card.remove(); });
        // Generate
        const genBtn = card.querySelector('.qw-generate-btn');
        if (genBtn) genBtn.addEventListener('click', () => generateQuote());
    }

    async function generateQuote() {
        const price = calcPrice();
        if (price.items.length === 0 || price.total <= 0) return;

        const genBtn = card.querySelector('.qw-generate-btn');
        if (genBtn) { genBtn.disabled = true; genBtn.textContent = '...'; }

        try {
            const res = await fetch(SUPA_URL + '/functions/v1/generate-quote-pdf', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + SUPA_KEY },
                body: JSON.stringify({ items: price.items, shipping_fee: price.shipping, lang: getLang() })
            });
            const data = await res.json();

            if (data.url) {
                const _total = (data.total || price.total || 0).toLocaleString();
                const _quoteId = 'quote_' + Date.now();
                window['_pendingQuote_' + _quoteId] = { items: price.items, products: [], shipping_fee: price.shipping };

                localStorage.setItem('chameleon_quote_pdf_url', data.url);

                // Save to chat_messages if room exists
                try {
                    const _qItemsSummary = price.items.map(i => `${i.name} ${i.spec||''} x${i.qty} = ${(i.total||0).toLocaleString()}${qw('won')}${i.is_addon?' ('+qw('addOpt')+')':''}`).join('\n');
                    const _quoteMsgText = '[QUOTE_PDF:' + data.url + ']\n[' + qw('title') + ']\n' + _qItemsSummary + '\n' + qw('total') + ': ' + price.total.toLocaleString() + qw('won') + (price.shipping ? ' + ' + qw('shippingFee') + ' ' + price.shipping.toLocaleString() + qw('won') : '');
                    const _sbQ = getSb();
                    if (_sbQ && _advRoomId) {
                        ...
                        }).then(r => { if (r.error) console.error('[Wizard] DB save error:', r.error); });
                    }
                } catch(_qe) {}

                // Remove wizard, show PDF card
                card.remove();
                ...
                _pdfCard.innerHTML = '<div style="font-size:14px;font-weight:800;margin-bottom:8px;">\u{1F4C4} ' + (lang === 'ja' ? '見積書が準備できました' : lang === 'en' ? 'Your quote is ready' : '견적서가 준비되었습니다') + '</div>'
                    + '<div style="font-size:12px;opacity:0.9;margin-bottom:12px;">' + qw('total') + ': ' + _total + qw('won') + ' (VAT' + (lang === 'ja' ? '込み' : lang === 'en' ? ' incl.' : '포함') + ')</div>'
                    + '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;">'
                    + '<a href="' + data.url + '" target="_blank" style="display:inline-flex;align-items:center;gap:6px;background:#fff;color:#4338ca;padding:10px 16px;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none;">\u{1F4E5} ' + (lang === 'ja' ? '見積書ダウンロード' : lang === 'en' ? 'Download Quote' : '견적서 다운로드') + '</a>'
                    + '<button onclick="window._quoteToCart(\'' + _quoteId + '\')" style="display:inline-flex;align-items:center;gap:6px;background:#22c55e;color:#fff;padding:10px 16px;border-radius:8px;font-size:13px;font-weight:700;border:none;cursor:pointer;">\u{1F6D2} ' + (lang === 'ja' ? '見積金額で決済' : lang === 'en' ? 'Pay Quote Amount' : '견적금액 결제하기') + '</button>'
                    + '</div>'
                    + '<div style="font-size:11px;opacity:0.85;line-height:1.6;">' + (lang === 'ja' ? '見積を確認後、決済ボタンを押すとカートに商品が入ります。' : lang === 'en' ? 'Review the quote and click pay to add items to your cart.' : '견적 확인하시고 결제하기를 누르시면 장바구니에 제품이 담겨있어요.') + '</div>';
                if (chatArea) { chatArea.appendChild(_pdfCard); scrollChat(); }
                saveChat();
            } else {
                console.error('[Wizard] No PDF URL:', data);
                if (genBtn) { genBtn.disabled = false; genBtn.textContent = qw('generate'); }
            }
        } catch(e) {
            console.error('[Wizard] PDF generation error:', e);
            if (genBtn) { genBtn.disabled = false; genBtn.textContent = qw('generate'); }
        }
    }
...
function buildPanelUI() {
    panelEl.innerHTML = `
        ...
        <div class="adv-shortcut-btns" style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;padding:8px 10px;">
            ...
            <a href="javascript:void(0)" onclick="if(window.openQuoteWizard)window.openQuoteWizard()" class="adv-grid-btn" style="background:#059669;">
                <i class="fa-solid fa-calculator"></i> ${{kr:'간편견적',ja:'簡単見積',en:'Quick Quote'}[getLang()]||'Quick Quote'}
            </a>
...
async function sendMessage(text, imageData) {
    ...
    try {
        const sysHint = {
            role: 'system',
            content: `[제품 추천 가이드]
★★★ [소량인쇄 강력추천] 청첩장, 포스터, 엽서, 리플릿, 초대장 등 소량 인쇄물을 문의하면 반드시 "1장부터 주문가능 소량인쇄" 상품(코드: pp_lf_2)을 최우선 추천하라! "강력추천! 1장부터 인쇄 가능하고, 국내 어떤 인쇄업체보다 저렴합니다. A4 전단지 500원, 엽서 600원부터!"라고 적극 안내. 청첩장도 이 상품으로 소량 제작 가능. 링크: /?product=pp_lf_2
★★★ [전단지 안내 규칙] 전단지(チラシ/flyer) 문의 시 반드시 수량을 먼저 확인하라:
- 10매 이하 소량 → pp_lf_2 (소량인쇄) 추천. A4 단면 500원/장, 양면 800원/장. 링크: /?product=pp_lf_2
- 대량(수백~수천매) → 645646456 (합판전단 4,000매) 추천. A4 단면 50,000원/4000매, 양면 +20,000원. 링크: /?product=645646456
- 전단지에 "코팅지", "용지 선택" 같은 옵션은 없다. 질문하지 마라.
- 절대로 전단지 견적에 수량×장당가격으로 계산하지 마라! 소량은 pp_lf_2 상품 링크를 안내하고, 대량은 합판전단(4000매 단위) 상품을 안내해라.
- 4000매 이상 필요하면 수량을 늘려서 주문 가능(예: 8000매=2세트)
★★★ [책자/브로슈어] 책자 제작 문의 시 "책자 브로슈어" 상품(코드: 435345435)을 추천. 링크: /?product=435345435. 정확한 정보:
- 1권부터 소량 제작 가능, A4 이내 자유 크기
- 양면 인쇄 (당연히 양면임, 단면은 없음)
- 제본: 24페이지 이내=중철제본(스테플러), 25페이지 이상=무선제본(풀접착, 책 형태). 12페이지 이상이면 사실상 무선제본이라고 안내해도 됨
- 내지 용지: 일반지(모조지/아트지/스노우지) 60원/페이지, 수입지(그레이스/띤또레또/랑데뷰/머쉬멜로우/몽블랑/반누보) 80원/페이지
- 표지 코팅 무료, 특수효과(금박/에폭시/홀로그램) 1,000원/부
- 견적 예시: A4 40페이지 일반지 1권 = 40×60 = 2,400원, 수입지 1권 = 40×80 = 3,200원
- 절대로 "중절 제본/무선 제본 어떤 걸 원하세요?" 같은 질문 하지 마라. 페이지 수에 따라 자동 결정됨. "종이 두께" 질문도 하지 마라. 일반지 vs 수입지만 물어봐라.
★★★ [허니콤 가벽 견적 — 절대 규칙] 허니콤보드 가벽/전시벽/파티션 견적 시 반드시 아래 순서를 따라라:
[STEP 1: 사이즈 규격 확인] 고객이 요청한 사이즈가 규격에 맞는지 먼저 확인:
  - 가로: 1미터 단위만 가능 (1m, 2m, 3m, 4m...). 규격 외(예:3.2m)면 → "죄송합니다. 가벽은 가로 1미터 단위로 규격화되어 있어서 3.2미터는 제작이 불가합니다. 3미터 또는 4미터로 다시 정해주세요." 절대로 규격 외 사이즈로 견적 만들지 마라! 고객이 다시 정할 때까지 기다려라.
  - 높이: 2000, 2200, 2400, 3000mm만 가능. 규격 외(예:1.8m, 2.5m)면 → "높이는 2m, 2.2m, 2.4m, 3m 중 선택해주세요." 2m 미만은 2m로 주문 후 커팅 가능(가격 동일)하다고 안내.
  ★ 사이즈가 규격에 맞을 때까지 견적을 만들지 마라!
[STEP 2: 추가옵션 확인] 사이즈가 확정되면 아래 3가지 옵션을 물어봐라:
  "추가 옵션도 확인할게요!
  🛡️ 보조받침대 — 야외나 아이들이 많은 곳에서는 안전을 위해 추천드려요
  💡 조명 — 가벽에 포인트 조명을 달아 분위기를 살릴 수 있어요
  🔲 코너기둥 — ㄴ자나 ㄱ자로 벽이는 공간 연출이라면 필요해요
  필요한 옵션이 있으시면 말씀해 주세요! 없으시면 '없음'이라고 해주세요."
[STEP 3: 견적 생성] 사이즈+옵션이 확정되면 단면 기본으로 견적 생성. 견적 후 반드시 이 문구 추가:
  "혹시 양면 인쇄이거나 지방 배송이시라면 견적이 달라지니 말씀해 주세요. 수정해서 다시 견적 드릴게요!"
  단면/양면을 사전에 묻지 마라. 배송지역도 사전에 묻지 마라. 견적 후에만 안내.
...
[중요] 인사나 잡담에는 제품을 추천하지 말고 짧고 재치있게 인사만 해라 (예: "안녕하세요! 무엇을 도와드릴까요?"). 구체적 질문이 있을 때만 관련 제품 3개를 추천할 것. 사용자가 구체적 제품을 물어도 해당 제품 + 관련 제품 2개를 함께 추천.`
        ...
        // ★ [QUOTE_WIZARD] 태그 감지 → 견적 위자드 표시
        const _hasQuoteWizard = chatMsg.includes('[QUOTE_WIZARD]');
        if (_hasQuoteWizard) chatMsg = chatMsg.replace(/\[QUOTE_WIZARD\]/g, '').trim();
        if (chatMsg) addBubble(chatMsg, 'ai');
        if (_hasQuoteWizard) setTimeout(() => { if (window.openQuoteWizard) window.openQuoteWizard(); }, 300);
