...
import { ADDON_DB as _IMPORTED_ADDON_DB, currentUser, sb } from "./config.js?v=294";
import { pageDataList, currentPageIndex } from "./canvas-pages.js?v=294"; // 페이지 인덱스 가져오기
import { FONT_URLS, FONT_ALIASES } from "./fonts.js?v=294";

// 2026-06-11: config.js 가 파일별로 ?v= 가 달라 모듈 인스턴스가 분리됨 (export.js?v=294 / index.html?v=439 등).
...
const ADDON_DB = new Proxy({}, {
    get(_, key) {
        const live = (typeof window !== 'undefined' && window.ADDON_DB) ? window.ADDON_DB : _IMPORTED_ADDON_DB;
        return live ? live[key] : undefined;
    },
    has(_, key) {
        const live = (typeof window !== 'undefined' && window.ADDON_DB) ? window.ADDON_DB : _IMPORTED_ADDON_DB;
        return live ? (key in live) : false;
    }
});
...
const PDF_LABELS = {
    ...
    zh: {
        ...
        phone: "联系方式 :",
        address: "地   址 :",
        provider_labels: ["统一社会信用代码", "公司名称", "代表人", "地   址", "行   业", "联系方式"],
        provider_values: ["88-4057291", "Chameleon Printing Inc.", "Ji-Woon Hong", "3F, 32-11 Yakkasaki 7-chome, Matsudo-shi, Chiba 270-0023", "制造业 / 服务业", "047-712-1148"],
    ...
    es: {
        ...
        provider_values: ["88-4057291", "Chameleon Printing Inc.", "Ji-Woon Hong", "3F, 32-11 Yakkasaki 7-chome, Matsudo-shi, Chiba 270-0023", "Fabricación / Servicios", "047-712-1148"],
    ...
    de: {
        ...
        provider_values: ["88-4057291", "Chameleon Printing Inc.", "Ji-Woon Hong", "3F, 32-11 Yakkasaki 7-chome, Matsudo-shi, Chiba 270-0023", "Herstellung / Dienstleistungen", "047-712-1148"],
    ...
    fr: {
        ...
        provider_values: ["88-4057291", "Chameleon Printing Inc.", "Ji-Woon Hong", "3F, 32-11 Yakkasaki 7-chome, Matsudo-shi, Chiba 270-0023", "Fabrication / Services", "047-712-1148"],
...
export async function generateProductVectorPDF(inputData, w, h, x = 0, y = 0, perPageSizes = false) {
    ...
    try {
        ...
        for (let i = 0; i < pages.length; i++) {
            ...
            // 목업 오브젝트 제거 + 대지 정리 (PDF에 불필요한 에디터 속성 제거)
            const filteredJson = { ...json };
            if (filteredJson.objects) {
                filteredJson.objects = filteredJson.objects
                    .filter(o => !o.isMockup && !o.excludeFromExport)
                    .map(o => o.isBoard ? { ...o, strokeWidth: 0, stroke: null, shadow: null } : o);
            }
